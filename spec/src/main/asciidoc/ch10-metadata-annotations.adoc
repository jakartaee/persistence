...
and must not be a reserved identifiers in the Jakarta Persistence query
language.
...
`NamedAttributeNode`, and may be placed on the persistent attribute
...
`NamedSubgraph`, and may be placed on the association represented
