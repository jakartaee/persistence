...
The following annotations are used to declare named entity graphs.
Entity graphs are specified in <<a2397>>.
...
entity graph specify the limits of the graph of associated attributes and
...
same name and one is a subgraph of the other, as specified by
`NamedEntityGraph.Subgraph` annotations.
...
persistent attributes with `NamedEntityGraph.AttributeNode` and
`NamedEntityGraph.Subgraph`, or by mixing these two approaches.
...
==== NamedEntityGraph.AttributeNode

The `NamedEntityGraph.AttributeNode` annotation is an alternative to
`NamedAttributeNode`. It may be placed on the persistent attribute
represented by the graph node instead of being nested inside the
...
The `graph` member specifies the name of the entity graph to which
the node belongs. It defaults to the entity name of the annotated
entity class.

==== NamedEntityGraph.Subgraph

The `NamedEntityGraph.Subgraph` annotation is an alternative to
`NamedSubgraph`. It may be placed on the association represented
by the subgraph instead of being nested inside the `NamedEntityGraph`
annotation.
...
The `graph` member specifies the name of the entity graph to which
the subgraph belongs. It defaults to the entity name of the annotated
entity class.

The `subgraph` member specifies the name of an entity graph rooted
at the associated entity type. If not explicitly specified, it
defaults to the name of the entity graph to which the node belongs.
...
The persistence provider should treat the annotation
