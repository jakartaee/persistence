...
If the `access` subelement of `persistence-unit-defaults` is not specified
in any XML mapping descriptor, the default `access` for managed classes in
the persistence unit is `FIELD`.
...
===== named-query, named-native-query [[named-query]]

A named query defined by a `named-query` or `named-native-query`
element is scoped to the persistence unit.

If the mapping files for a persistence unit declare multiple queries
with the same name, the behavior is undefined.

A named query declared in XML is added to the named queries declared
using annotations. If a named query declared using annotations has
the same name as the query declared in XML, the declaration in XML
overrides the declaration in annotations.

===== named-statement, named-native-statement [[named-statement]]

A named statement defined by a `named-statement` or
`named-native-statement` element is scoped to the persistence unit.

If the mapping files for a persistence unit declare multiple statements
with the same name, the behavior is undefined.

A named statement declared in XML is added to the named statements
declared using annotations. If a named statement declared using
annotations has the same name as the statement declared in XML,
the declaration in XML overrides the declaration in annotations.

===== named-stored-procedure-query

A named stored procedure query defined by `named-stored-procedure-query`
element is scoped to the persistence unit.

If the mapping files for a persistence unit declare multiple stored
procedure queries with the same name, the behavior is undefined.

A named stored procedure query declared in XML is added to the named
stored procedure queries declared using annotations. If a named stored
procedure query declared using annotations has the same name as the
stored procedure query declared in XML, the declaration in XML overrides
the declaration in annotations.
...
The name attribute specifies the entity name.
...
The `pre-persist`, `post-persist`, `pre-remove`, `post-remove`,`pre-merge`,
`pre-insert`, `post-insert`, `pre-delete`, `post-delete`, `pre-upsert`,
`post-upsert`, `pre-update`, `post-update`, and `post-load` subelements
override any lifecycle callback methods defined by the corresponding
...
remainder of the attributes are defaulted according to the default
...
The `excluded-from-versioning` subelement overrides the
...
`post-upsert`, `pre-update`, `post-update`, and `post-load` subelements
override any lifecycle callback methods defined by the corresponding
annotations on the mapped superclass.
...
the remainder of the attributes are defaulted according to the
