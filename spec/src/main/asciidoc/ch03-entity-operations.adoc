...
When a transaction is active and the persistence context is joined
to the transaction, the persistence provider runtime is permitted to
perform synchronization to the database of modifications made to
entities associated with the persistence context. This behavior is
controlled by the `flush()` method of `EntityManager` and by the
specified `FlushModeType`.

- The `flush()` method can be called by the application to force
  synchronization of every modification of every entity associated
  with the persistence context.
- The `setFlushMode` methods of the `EntityManager`, `Query`,
  `TypedQuery`, and `StoredProcedureQuery` interfaces can be used
  to control the behavior  of automatic synchronization.
  * The effect of `FlushModeType.AUTO` is defined in <<a4374>>.
  * If `FlushModeType.COMMIT` is specified, the persistence context
    is flushed when the transaction commits; the persistence provider
    is permitted, but not required, to flush at other times.
...
must not flush modifications to the database.
...
- If `FlushModeType.AUTO` is selected via the `Query`, `TypedQuery`,
  or `StoredProcedureQuery` object, or if the flush mode setting
  for the persistence context is `AUTO` (the default) and a flush
...
- If `FlushModeType.COMMIT` or `FlushModeType.EXPLICIT` is selected,
  the effect on query results of unflushed modifications made to
  entities held in the persistence context is unspecified.
