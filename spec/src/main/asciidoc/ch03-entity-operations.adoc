...
[source,java]
----
include::../../../../api/src/main/java/jakarta/persistence/ManagedEntityMode.java[lines=61..-1]
----
...
may be loaded in read-only mode by passing `ManagedEntityMode.READ_ONLY`
to any method of `EntityManager` which accepts a `FindOption` or to
the `setManagedEntityMode()` method of `Query`. Furthermore, the current
mode for a given managed entity may be changed by calling the method
`setManagedEntityMode()` of `EntityManager`.

Changes made to an entity instance currently loaded in read-only mode
are not synchronized to the database and do not become persistent
The provider is not required to detect modifications to entities in
read-only mode.

When `READ_ONLY` is passed as an option, or when a query is executed
in `READ_ONLY` mode, as determined by the return value of the method
`getManagedEntityMode()` of `Query`, every entity loaded into the
persistence context during the invocation is loaded in read-only mode,
including any eagerly fetched associated entities. Entities which are
already loaded remain in their predetermined `ManagedEntityMode`.

On the other hand, when the `ManagedEntityMode` of an entity is changed
via invocation of the `setManagedEntityMode()` method of `EntityManager`,
the new mode applies only to the given managed entity instance, and does
not affect associated entities.

A managed entity in read-only mode has its mode automatically reset to
`READ_WRITE` when:

- it is directly returned by a call to a method of `EntityManager`,
  and `READ_ONLY` was not specified as an argument to the method,
- it is referenced by the entity returned by a call to a method of
  `EntityManager` via an association path marked for eager fetching
  within the fetch graph in effect during execution of the method,
  and `READ_ONLY` was not specified as an argument to the method,
- it occurs directly in the result of a query which was executed in
  `READ_WRITE` mode, as determined by the return value of the method
  `getManagedEntityMode()` of `Query`,
- it is referenced via an association path marked for eager fetching
  by an entity occurring directly in the result of a query which was
  executed in `READ_WRITE` mode, or
- it is passed as an argument to a `refresh()`, `lock()`, `persist()`,
  or `remove()` method of `EntityManager`.

If an entity in read-only mode is modified, and then the entity is reset
to modifiable mode, either automatically or explicitly, the behavior is
undefined. Such modifications might be lost; they might be made persistent;
or the provider might throw an exception. Portable applications should not
...
The `@PrePersist` and `@PreRemove` callbacks occur before the
respective persist and remove operations for an entity are executed.
For entities to which the `merge()` operation has been applied and
causes the creation of newly managed instances, the `@PrePersist`
callback occurs for the managed instance after the entity state has
been copied to it. The `@PrePersist` and `@PreRemove` callbacks also
occur for any entity to which these operations cascade. The `@PrePersist`
and `@PreRemove` methods are always invoked as part of the synchronous
`persist()`, `merge()`, and `remove()` operations of `EntityManager`.

The `@PostPersist` and `@PostRemove` callbacks occur after an entity
has been made persistent, or after it has been removed, respectively.
These callbacks also occur for any entity to which these operations
cascade. The `@PostPersist` and `@PostRemove` methods occur after the
database insert and delete operations, respectively. The database
operations might occur directly after invocation of `persist()`,
`merge()`, or `remove()`, or they might occur during a `flush()`
operation--which may not happen until the end of the transaction.
...
The `@PostLoad` callback occurs after an entity instance has been loaded
from the database by any means, or after the `refresh()` operation has
been applied to an entity instance. The `@PostLoad` method is invoked
before the newly-loaded query entity is returned or made accessible to
the application.
