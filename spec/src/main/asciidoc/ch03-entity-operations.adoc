...
method returns true if the argument is a managed entity instance associated
with the given persistence context, and is not marked as removed.
...
- For an `EntityManager`, every entity instance returned by any
  ...
  from the manager, is in the managed state and belongs to the
  persistence context associated with the entity manager. The instance
  remains in the managed state until that entity manager is closed and
  its persistence context is destroyed. An `EntityManager` is required
  to ensure that if an entity instance representing a given record
  already exists in its associated persistence context, then exactly
  that instance is returned by every operation that returns an entity
