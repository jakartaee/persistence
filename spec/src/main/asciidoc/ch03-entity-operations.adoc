...
+
 void <METHOD>(S)
+
where _S_ is any supertype of the entity class or mapped superclass to
...
access, but must not be _static_ or _final_.
...
* In general, the lifecycle method of a portable application should not
  invoke _EntityManager_ or query operations, access other entity
  ...
  objects that might be injected into an entity listener]footnote:[The
  semantics of such operations may be standardized in a future release of
  this specification.].
