...
automatically for every entity with a version attribute, as defined in
<<a2060>>. A portable application which wishes to take advantage of
automatic optimistic locking must specify a version attribute for each
...
If an entity does not have a version attribute, the application itself
must bear the burden of maintaining data consistency during optimistic
units of work.

==== Version Attributes and Optimistic Locking [[a2059]]
