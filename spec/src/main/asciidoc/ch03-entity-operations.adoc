...
Changes made to an entity instance currently loaded in read-only mode are
not synchronized to the database, and do not become persistent.

When `READ_ONLY` is passed as an option or to `Query.setManagedEntityMode()`,
every entity loaded into the persistence context during the invocation, or
during subsequent execution of the `Query`, is loaded in read-only mode,
including any eagerly-fetched associated entities.

On the other hand, when the `ManagedEntityMode` of an entity is changed
via a call to `EntityManager.setManagedEntityMode()`, the new mode applies
only to the given managed entity instance, and does not affect associated
entities.

If a managed entity loaded in read-only mode would be returned by a method
of `EntityManager` which does not request `READ_ONLY` mode, or among the
results of a query which does not request `READ_ONLY` mode, a
`PersistenceException` is thrown.

If an entity loaded in read-only mode is passed to the `refresh()` method
of `EntityManager`, its mode is automatically reset to `READ_WRITE`. If
an entity loaded in read-only mode is passed to the `remove()` or `lock()`
method of `EntityManager`, a `PersistenceException` is thrown.
