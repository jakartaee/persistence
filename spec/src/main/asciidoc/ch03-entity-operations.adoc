...
- the select list of the query contains only a single item, which must
  be assignable to the given result class, or
- the result class must be a non-`abstract` class or record type with a
  constructor with the same number of parameters as the query has items
  in its select list, and the constructor parameter types must exactly
  match the types of the corresponding items in the select list.
...
- the result class is the class of a basic type listed in <<a486>> and
  ...
  scalar result as specified in <<scalar-results>>, or
- the result class must be a non-`abstract` class or record type with a
  ...
  columns of the result set as specified in <<constructor-results>>.
