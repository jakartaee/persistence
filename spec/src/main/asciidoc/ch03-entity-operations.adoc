...
- if the entity manager has an extended persistence context, the returned
  entity or entities are managed.

Similar rules applying to entity instances returned by execution of a query
via an `EntityManager` are specified in <<query-execution-transactions>>.
...
any query obtained by calling `createQuery()`, `createNamedQuery()`,
