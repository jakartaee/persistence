...
The `find()` method may be invoked when there is no active transaction only
if it is called without an explicit lock mode or with the explicit mode
`LockModeType.NONE`.

- If the entity manager has a transaction-scoped persistence context, the
  entity returned by `find()` is detached;
...
via an `EntityManager` are specified in <<query-execution>>.
