...
the persistence context is joined to the transaction. The application
can influence the timing of synchronization via the `flush()` method of
`EntityManager` or by setting the `FlushModeType`.
