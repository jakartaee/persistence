...
entities and their relationships as specified above.
...
- The `flush()` method of `EntityManager` can be used by the application
   ...
   associated with the persistence context to the database.
...
Every query executed using `getResultList()`, `getSingleResult()`,
or `getSingleResultOrNull()` has a well-defined result type.
...
the methods `getResultList()`, `getSingleResult()`, and `getSingleResultOrNull()`
may be called when no transaction is activefootnote:[A lock mode may be
specified for a query by calling `setLockMode()` or by specifying the
`lockMode` member of the `@NamedQuery` annotation.].
...
- a runtime exception thrown by the following methods of the `Query`,
  `TypedQuery`, and `StoredProcedureQuery` interfaces do not cause the
  current transaction to be marked for rollback:
...
A named parameter of a Jakarta Persistence query language query is an
identifier that is prefixed by the `:` symbol. Parameter names passed
as arguments to the `setParameter` methods of the `Query` interface do
