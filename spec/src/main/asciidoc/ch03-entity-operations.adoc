...
`Query`, which are used to control query execution. `Query`, `TypedQuery`,
`StoredProcedureQuery`, and related interfaces are described in <<a3125>>.
The Jakarta Persistence query language is defined in <<a4665>> and APIs
for the construction of Criteria queries in <<a6925>>. <<a2397>> describes
the use of entity graphs to control and limit the data fetched during find
...
Any `Query`, `TypedQuery`, `StoredProcedureQuery`, `CriteriaBuilder`,
`Metamodel`, or `EntityTransaction` object obtained from a given entity
manager or entity agent is valid only while that entity manager or entity
agent is open. A method of `Query`, `TypedQuery`, or `StoredProcedureQuery`
...
representing a Jakarta Persistence Query Language query of Criteria query.
An `IllegalArgumentException` may be thrown by `createQuery()` if:

- the argument to a `createQuery()` is not a legal Jakarta Persistence
...
- the argument to a `createQuery()` is not a valid `CriteriaQuery` object.

Alternatively, `createQuery()` might return a result, and then execution of
the query might later fail. In that case, a `PersistenceException` is thrown
when the query is executed.

The method `createNativeQuery()` is used to obtain a `Query` or `TypedQuery`
representing a native SQL query. A `PersistenceException` is thrown when the
native query is executed if:

- the query is rejected by the database server, or
...
The persistence provider runtime is permitted to perform synchronization
to the database at other times, as long as a transaction is active and
the persistence context is joined to the transaction.

- The `flush()` method of `EntityManager` can be used by the application
   to force synchronization. When `flush()` is called, the provider must
   synchronize every pending modification made to every entity currently
   associated with the persistence context.

- The `setFlushMode` method of the `EntityManager`, `Query`,
...
(entities, mapped superclasses, and embeddable classes)
at the entity lifecycle events described in <<a2202>>.

The Jakarta Persistence provider may automatically enforce
the Jakarta Validation constraints <<a19498>> defined on managed objects (<<a2374>>).
This process occurs internally within the persistence lifecycle,
ensuring that validation is performed without requiring additional application-side logic
beyond the definition of the constraints themselves.

==== Automatic Validation Upon Lifecycle Events

_Automatic validation_ of entities is specified in terms of their lifecycle events
defined in <<a2202>>.

If enabled, automatic validation of entities must occur immediately after the point at
which all corresponding lifecycle callback method invocations complete, or immediately
after the point at which such lifecycle callback methods would have completed if there
were any such callback methods present.

[NOTE]
====
Automatic validation is triggered as an immediate post-action of a lifecycle callback.
When multiple operations are applied to a single entity within the scope of a single transaction,
the number of automatic validation events is implementation-dependent
and may vary across Jakarta Persistence providers.

For example, it is provider-specific whether a `PreUpdate` callback is invoked
if an entity is persisted and subsequently modified, or modified and subsequently removed,
within the same transaction <<a2202>>.
Consequently, the automatic validation associated to the pre-update lifecycle callback
may or may not occur in these scenarios.
...
* `callback` -- the persistence provider must perform the automatic validation of entities
  as described in <<a2380>>. If no Jakarta Validation provider is available,
  the persistence provider must throw a `PersistenceException`.
...
The `Query` and `TypedQuery` interfaces are used to control parameter
binding and execution for both:

- static statements and queries declared in metadata, and
- dynamic statements and queries passed to methods of `EntityHandler`.

These interfaces also support pagination control and control over locking.
...
A query or statement written in the Jakarta Persistence query language
or in native SQL is executed via the `Query` or `TypedQuery` interface.
...
- A `SELECT` query specified in the Jakarta Persistence query language
  ...
  `getSingleResult()`, or `getSingleResultOrNull()`.
...
A stored procedure that does not return a result set can be executed
...
The `executeUpdate()` of `Query` or `TypedQuery` must not be called
without an active transaction contex.
...
exception thrown by any method not explicitly listed below of `Query`,
`TypedQuery`, or `StoredProcedureQuery` causes the current transaction
to be marked for rollback, unless the exception is an instance of one
of the following exception types:
...
- a runtime exception thrown by the following methods of the `Query`,
  `TypedQuery`, and `StoredProcedureQuery` interfaces does not cause
...
a transaction via a `Query`, `TypedQuery`, or `StoredProcedureQuery`
object obtained from an `EntityManager` joined to the transaction,
the effective flush mode is determined by the current flush mode of
the `Query` object, if any flush mode has been specified by calling
`setFlushMode()`, or, otherwise, by the current flush mode of the
persistence context.
