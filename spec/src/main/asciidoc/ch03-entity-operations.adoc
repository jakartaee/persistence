...
binding and execution for both static statements and queries declared
in metadata and dynamic statements and queries passed to methods of
`EntityHandler` as strings. These interfaces also support pagination
control. The `StoredProcedureQuery` interface is used to call stored
procedures defined in the database.
