...
to `EntityManager.find()` or `Query.setManagedEntityMode()`. Furthermore,
...
When `READ_ONLY` is passed to `find()` or to `Query.setManagedEntityMode()`,
every entity loaded into the persistence context during the invocation of
`find()` or subsequent execution of the `Query` is loaded in read-only mode,
including any eagerly-fetched associated entities

When a `ManagedEntityMode` is passed to `EntityManager.setManagedEntityMode()`,
the mode applies only the given managed entity instance, and does not affect
associated entities.

If a managed entity loaded in read-only mode would be returned by a call
to `find()` which does not request `READ_ONLY` mode, or among the results
of a query which does not request `READ_ONLY` mode, a `PersistenceException`
must be thrown by `find()` or by the method which executes the query.
