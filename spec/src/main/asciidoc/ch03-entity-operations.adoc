...
contexts_, as defined below in <<a2027>> and <<a11431>>. In this augmented
programming model, the application holds a reference provided by the Jakarta
EE container to an instance of `EntityManager` whose underlying persistence
context is determined by transaction affinity.footnote:[This is accomplished
via indirection at the container level. Every transaction-scoped persistence
...
- the persistence context is a container-managed transaction-scoped
  persistence context, as defined in <<a2027>>, and the transaction
  associated with the persistence context commits, as specified in
  <<a2048>>,
...
than `LockModeType.NONE` has been specified for the query. In particular,
if no lock is requested, the methods `getResultList()`, `getSingleResult()`,
and `getSingleResultOrNull()` may be called when no transaction is
activefootnote:[A lock mode may be specified for a query by calling
`setLockMode()` or by specifying the `lockMode` member of the `@NamedQuery`
annotation.].
...
- If the `Query` or `TypedQuery` object was obtained from an `EntityAgent`
  ...
  the entities returned by the query are detached;
- otherwise, if the query object was obtained or from an `EntityManager`
  with an extended persistence context, the returned entities are managed.

See <<a2027>> for information about transaction-scoped and extended
persistence contexts. See <<a11431>> for further discussion of entity
manager use with no active transaction and persistence context types.
