...
given _persistent entity identity_ defined by an entity type and a primary
key value, as specified above in <<a132>> and below in <<managed-entities>>.
footnote:[Alternatively, the context is a map keyed by persistent identity.]
The entity instances associated with a persistence context are considered
managed objects, with a well-defined lifecycle under the control of the
...
A newly instantiated instance is never persistent, and the new instance
