...
- The `setFlushMode` method of the `EntityManager`, `Query`, `Statement`,
  ...
  * If `FlushModeType.COMMIT` is specified, flushing will occur at
    ...
    required, to perform to flush at other times.
...
The `Query`, `Statement`, and `TypedQuery` interfaces are used to
control parameter binding and execution for both static statements
and queries declared in metadata and dynamic statements and queries
passed to methods of `EntityHandler` as strings. The `TypedQuery`
interface also supports pagination control. The `StoredProcedureQuery`
interface is used to call stored procedures defined in the database.
...
`getSingleResult`, and `getSingleResultOrNull` of `TypedQuery`,
or, for backward compatibility, using the identical deprecated
methods declared by `Query`. Update and delete statements are
executed using the `execute` method of `Statement` or, for
backward compatibility, using the deprecated `executeUpdate`
method of `Query`.
...
`TypedQuery` must not be called without an active transaction contex.
...
- a runtime exception thrown by the following methods of the `Query`,
  `Statement`, `TypedQuery`, and `StoredProcedureQuery` interfaces do
...
==== Result Classes for Jakarta Persistence Queries
