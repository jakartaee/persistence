...
The application may access the available state of available detached
entity instances after the persistence context ends. The available
state includes any persistent field or property:

* not marked `fetch=LAZY`, or
...
state of an associated instance may only be safely accessed if the
associated instance is available. The available instances include
any entity instance:

* retrieved using `find()`,
* retrieved using a query or explicitly requested in a fetch join or
  entity graph,
* with a persistent attribute other than the primary key which was
  accessed by the application, or
* reachable from an available instance by navigating associations marked
  `fetch=EAGER`.
...
instances when merging.

===== Detached Entities, Serialization, and Lazy Loading
...
A persistence provider is permitted  to report an exception if an entity
instance with unfetched lazy attributes instantiated by a different
persistence provider runtime is passed to `merge()`.
...
==== Managed Instances [[managed-entities]]
