...
If a method of `EntityManager` accepts a lock mode and the method is called
...
manager has not been joined to the transaction, and any lock mode other than
...
invoked when there is no active transaction. However, when there is no
active transaction, these methods may not be called with any explicit lock
mode other than `LockModeType.NONE` as an argument.
...
described above in <<entity-manager-transactions>> and below in
...
The operations `persist()`, `remove()`, `merge()`, and `refresh()`
may be called on an `EntityManager` with an extended persistence context
even when no transaction is active. The effect of such an operation is
never synchronized with the database, as defined in <<a1955>>, until
the extended persistence context is joined to an active transaction.
