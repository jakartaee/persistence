...
The `@PrePersist`, `@PreRemove`, or `@PreMerge` callback occurs
before the respective persist, remove, or merge operations for an
entity is executed. The `@PreMerge` callback occurs before any state
is copied from the entity. For entities to which the `merge()`
operation has been applied and causes the creation of newly managed
instances, the `@PrePersist` callback occurs for the managed instance
after the entity state has been copied to it. The `@PrePersist`,
`@PreRemove` and `@PreMerge` callbacks also occur for any entity to
which these operations cascade. The `@PrePersist` and `@PreRemove`
methods are always invoked as part of the synchronous `persist()`,
`merge()`, and `remove()` operations of `EntityManager`.
