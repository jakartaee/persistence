...
entity instances in which for any given _persistent entity identity_, defined
by an entity type and a primary key value, as specified in <<a132>>, there
is at most one entity instance. The entity instances associated with a
persistence context are considered managed objects, with a well-defined
...
contexts_, as defined below in <<a11431>>. In this augmented programming
model, the application holds a reference provided by the Jakarta EE
container to an instance of `EntityManager` whose underlying persistence
...
* if `e` is a detached object, the `EntityExistsException` may be thrown
