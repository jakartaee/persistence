...
When a lifecycle event occurs for an entity, the persistence
...
be loaded.
* Cascading validation (`@Valid`) must not
...
`jakarta.validation.TraversableResolver` interface to the Jakarta Validation
provider by calling
