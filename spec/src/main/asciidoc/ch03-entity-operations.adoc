...
argument to the `remove()` method method of `EntityManager` or when the
...
* if `e` is a new entity, `e` itself ignored by the remove operation;
...
* If `e` is a managed entity:
  ...
  - Every entity `t` referenced by a relationship owned by `e` which
    ...
    *** if `t` is detached, the semantics depend upon the ownership of the
        ...
        owns the relationships, the behavior is undefined.
* If `e` a removed entity, the record represented by `e` is removed from
...
A persistence provider is permitted report an exception if an entity
...
`EntityHandler`, `EntityManager`, or `EntityAgent` depend on whether
