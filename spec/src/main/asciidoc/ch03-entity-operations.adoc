...
to associated entities. The cascade functionality is most typically used
...
* if `e` is a new entity, it becomes managed and a new record represented
  ...
  as a result of the flush operation specified below in <<a1955>>;
...
A managed entity instance becomes removed when it is passed as an
argument to the `remove()` method of `EntityManager` or when the
remove operation cascades from another instance to the managed instance.
...
* If a managed entity instance is removed from the relationship by setting
  ...
  as defined below in <<a1955>>. If an entity being orphaned is a detached,
...
* If the remove operation is applied to a managed entity, the remove operation
  is cascaded to the relationship target in accordance with the rules of <<a1946>>,
...
information about synchronization may be found above in <<a1955>>.
...
an operation is never synchronized with the database, as defined in <<a1955>>,
