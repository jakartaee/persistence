...
argument to the `persist()` method of `EntityManager`, or if the persist
...
Let `e` be an instance of an entity class. If `e` is passed as the first
argument to the `persist()` method of `EntityManager`, or if the remove
operation is applied to `e` via cascade:
...
argument to the `detach()` method of `EntityManager`, or if the detach
operation is applied to `e` via cascade:
