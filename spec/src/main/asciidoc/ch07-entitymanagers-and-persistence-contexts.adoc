...
and transaction management with an application-managed `EntityHandler`.
...
By default, the `EntityHandler` is an `EntityManager`. The caller may
request a different subtype of `EntityHandler` by passing the Java
class object representing the subtype as the first argument. This
class may be `EntityAgent`, `EntityManager`, or some provider-specific
subtype of `EntityHandler`.

The argument function passed to `runInTransaction` or
`callInTransaction` must be called and passed a new instance of
`EntityHandler`. When the argument function returns or throws an
exception, this `EntityHandler` must be closed before `runInTransaction`
or `callInTransaction` returns.

The argument function is executed in the context of a transaction
associated with this new `EntityHandler`.
...
- If the transaction type of the persistence unit is JTA, and there
  ...
  `EntityHandler` is associated with this current transaction. If the
...
- Otherwise, if the transaction type of the persistence unit is
  ...
  with the caller, then the `EntityHandler` is associated with a new
...
transaction or `EntityHandler` directly. If the application calls an
