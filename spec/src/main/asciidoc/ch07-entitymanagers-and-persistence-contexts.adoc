...
In Jakarta EE environments, a JTA (Jakarta Transactions)<<a19501>> transaction
...
are required to be supportedfootnote:[Note that the use of
...
==== Obtaining an Entity Manager in the Jakarta EE Environment [[a11464]]
...
==== Obtaining an Entity Agent in the Jakarta EE Environment [[a11465]]

A container-managed entity agent is obtained by the application through
dependency injection or through direct lookup of the entity agent in the
JNDI namespace. The container manages its lifecycle and the creation and
the closing of the entity agent instance transparently to the application.

The `PersistenceAgent` annotation is used for entity agent injection.
The `unitName` element may optionally be specified to designate the
persistence unit whose entity manager factory is used by the container.
Section <<a13887>> provides further information about the `unitName`
element.
...
The JNDI lookup of an entity agent is illustrated below:

[source,java]
----
@RequestScoped
@PersistenceAgent(name="OrderEA")
public class MyBean implements MyInterface {

    public void doSomething() {
        EntityAgent em = (EntityAgent) ctx.lookup("java:comp/env/OrderEA");
...
==== Obtaining an Application-managed Entity Manager or Entity Agent
...
than one database, since it is typical that each entity manager communicates
which exactly one database. On the other hand, there is usually only one entity
manager factory for each persistence unit.]

Methods of the `EntityManagerFactory` interface are threadsafe.

==== Obtaining an Entity Manager Factory in a Jakarta EE Container [[a11463]]
...
Outside a Jakarta EE container environment, the `Persistence` class
described in <<a13443>> is the bootstrap class providing access to
an entity manager factory. The application creates an entity manager
factory by calling the `createEntityManagerFactory` method of the
`Persistence` class.
...
The class `PersistenceConfiguration` described in <<a13444>> may be
used to programmatically define and configure a persistence unit
(see <<a12229>>), as an alternative to packaging a `persistence.xml`
file, mapping files, and classes inside an archive as described in
<<a12236>>.
...
Sections <<a11805>> and <<a11810>> describe transaction-scoped and
extended persistence contexts in the absence of persistence context
propagation. Persistence context propagation is described in <<a11816>>.
...
that transaction according to the rules in <<a11820>> regardless of whether
the persistence context has been joined to that transaction.

==== Container-managed Transaction-scoped Persistence Context [[a11805]]

The application can obtain a container-managed entity manager with
transaction-scoped persistence context by injection or direct lookup
in the JNDI namespace. The persistence context type for the entity
manager must be defaulted or explicitly specified as
`PersistenceContextType.TRANSACTION`.

A new persistence context begins when the container-managed entity
manager is invokedfootnote:[That is, when a method of the
`EntityManager` interface is invoked.] in the scope of an active
JTA transaction, and there is no current persistence context already
associated with the transaction. The persistence context is created
and associated with the transaction. This affinity of the persistence
context with the JTA transaction is independent of the synchronization
type of the persistence context and of whether the persistence context
has been joined to the transaction.

The persistence context ends when the associated transaction commits or
rolls back, and all entities that were managed by the `EntityManager`
become detached.footnote:[This also applies to a transaction-scoped
persistence context of type `SynchronizationType.UNSYNCHRONIZED` that
...
within the scope of a stateful session bean which declares as dependency
on the extended persistence context by means of the `PersistenceContext`
annotation or `persistence-context-ref` deployment descriptor element.

The extended persistence context exists from the point at which the
stateful session bean declaring a dependency on the persistence context
is created and is said to be _bound_ to the stateful session bean.

The container destroys the extended persistence context after the
`@Remove` method of the stateful session bean completes or when the
...
This rule applies recursively, independently of whether transactions
are active at the moment of the creation of the stateful session beans.
If the stateful session beans differ in declared synchronization type,
an `EJBException` is thrown by the container.

The container does not destroy an extended persistence context until
every stateful session bean inheriting the persistence context has been
...
to the transaction. Note that the JTA transaction may roll back in a
background thread (e.g., as a result of transaction timeout), in which
...
corresponding action. The method `populate()` triggers data loading
as described in <<a12918>> and corresponds to the action `populate`.
