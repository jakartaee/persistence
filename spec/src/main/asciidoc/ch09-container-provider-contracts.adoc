...
`getManagedPackageNames()`, and `getManagedModuleNames()` methods report the
entries explicitly declared by the corresponding `class`, `package`, and
`module` elements of `persistence.xml`. The `getAllClassNames()`,
`getAllPackageNames()`, and `getAllModuleNames()` methods report the complete
categorized persistence-unit view obtained from explicit declarations, mapping
metadata, and container discovery, as applicable. None of these lists contains
duplicates.
...
The `managedClass()`, `managedPackage()`, and `managedModule()` operations
correspond to the `class`, `package`, and `module` elements of
`persistence.xml`. A package is specified by its qualified package name without
