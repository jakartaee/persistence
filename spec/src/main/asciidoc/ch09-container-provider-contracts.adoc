...
database artifacts. The values for this property are `none`,
`create`, `drop-and-create`, `drop`, `validate`, `populate`. If the
...
specified. If this property is not specified, no scripts are
generated.
...
generally expected that the container is responsible for executing
