...
as a result is used by the container to create container-managed
...
The container must make the `EntityManager`, `EntityAgent`, and
`EntityManagerFactory` available for lookup in JNDI and injection via
the `@PersistenceContext` and `@PersistenceUnit` annotations. The
`name` of the persistence unit, as specified in <<name>>, determines
the JNDI addresses of these objects. For a persistence unit with name
`unitName` located in a WAR, EJB-JAR, or application client JAR:

- the `EntityManager` is bound to the address
  `java:module/persistence/unitName/EntityManager`,
- the `EntityAgent` is bound to the address
  `java:module/persistence/unitName/EntityAgent`, and
- the `EntityManagerFactory` is bound to the address
  `java:module/persistence/unitName/EntityManagerFactory`.

For a persistence unit with name `unitName` located directly in an
EAR:

- the `EntityManager` is bound to the address
  `java:app/persistence/unitName/EntityManager`,
- the `EntityAgent` is bound to the address
  `java:app/persistence/unitName/EntityAgent`, and
- the `EntityManagerFactory` is bound to the address
  `java:app/persistence/unitName/EntityManagerFactory`.

The `name` member of the `@PersistenceContext` or `@PersistenceUnit`
annotation or the corresponding element of the XML deployment
descriptors may be used to assign a name to in the `java:comp/env`
JNDI namespace.

The application may obtain a reference to these objects via injection
or JNDI lookup, as specified in <<a11464>> and <<a11463>>.
...
In cases where a preconfigured database (or “legacy” database) is not
in use or is not available, the Jakarta Persistence schema generation
facility may be used to generate the tables and other database artifacts
required by the persistence application. Whether schema generation entails
the creation of schemas proper in the database is determined by the
environment and by the configuration of the schema generation process,
as described below.

Schema generation may happen either prior to application deployment or
when the `EntityManagerFactory` is created as part of the application
deployment and initialization process.

* In Jakarta EE environments, the container may call the static method
  `PersistenceProvider.generateSchema()` separately from and/or prior
  to the creation of the entity manager factory for a persistence unit,
  or the container may pass additional information to the
  `createContainerEntityManagerFactory()` call that causes schema
  generation to happen as part of the entity manager factory creation
  and application initialization process.
* In Java SE environments, the application may call the static method
  `Persistence.generateSchema()` method separately from and/or prior
  to the creation of the entity manager factory or may pass information
  to the `createEntityManagerFactory()` method to cause schema
  generation to occur as part of the entity manager factory creation.

The information passed to these methods in the form of property settings
controls whether:

- generation occurs directly in the target database,
- DDL scripts for schema generation are created as files,
- or both.

==== Properties for Schema Generation

The application may provide DDL scripts to be used for schema generation,
as described in <<a12384>>. The application developer may package these
scripts as part of the persistence unit or may specify file URLs identifying
the location of such scripts as strings.

- In Jakarta EE environments, such scripts may be executed by the container,
  or the container may direct the persistence provider to execute the scripts.
- In Java SE environments, execution of the scripts is the responsibility of
  the persistence provider.

In the absence of the specification of scripts, schema generation, if requested,
is determined by the object/relational metadata of the persistence unit.

The following standard properties are defined for configuring the schema
generation process.

- In Jakarta EE, these properties are passed by the container in the `Map`
  argument of either the `generateSchema()` method or the
  `createContainerEntityManagerFactory()` method of `PersistenceProvider`.
- In Java SE, they are passed in the `Map` argument of either the
  `generateSchema()` method or the `createEntityManagerFactory` method
  of `Persistence`.

In Jakarta EE environments:

- Every string representing the file URL of a script source or target
  must specify an absolute path. If a file URL is specified as a relative
  path, the behavior is undefined.
- Every source file and target file location must be accessible to the
  application server deploying the persistence unit.


[NOTE]
====
This specification does not define the behavior if a target location for
a script is already occupied by an existing file. An implementation might
overwrite the file, append to the file, or fail fast. Portable applications
must delete or truncate any file existing at a script target location.
====

`jakarta.persistence.schema-generation.database.action` ::
This property specifies the schema management action to be performed by the
persistence provider on database artifacts. The values for this property are
`none`, `create`, `drop-and-create`, `drop`, `validate`, `populate`. If this
property is not specified, the provider must not perform schema management
actions on the database.

`jakarta.persistence.schema-generation.scripts.action` ::
This property specifies which scripts are to be generated by the persistence
provider. Legal values for this property are `none`, `create`, `drop-and-create`,
`drop`. If this property is not specified, no DDL scripts are generated.

`jakarta.persistence.schema-generation.create-source` ::
This property specifies whether the creation of database artifacts is based
on the object/relational mapping metadata, a DDL script, or a combination.
Legal values for this property are `metadata`, `script`, `metadata-then-script`,
`script-then-metadata`. When this property is not specified:
+
* if a script is specified by the property
  `jakarta.persistence.schema-generation.create-script-source`, the script
  (only) is executed to create database artifacts;
* otherwise, if that property is not specified, the provider infers a schema
  from the object/relational mapping metadata (only) and creates the artifacts
  of the inferred schema.

+
The values `metadata-then-script` and `script-then-metadata` specify that a
combination of metadata and script is used and the order in which this use
occurs. If either of these values is specified and the resulting database
actions are not disjoint, the results are undefined and the creation of
database artefacts might fail.

`jakarta.persistence.schema-generation.drop-source` ::
This property specifies whether the dropping of database artifacts is based
on the object/relational mapping metadata, a DDL script, or a combination.
Legal values for this property are `metadata`, `script`, `metadata-then-script`,
`script-then-metadata`. When this property is not specified:
+
* if a script is specified by the property
  `jakarta.persistence.schema-generation.drop-script-source`, the script (only)
  is executed to drop database artifacts;
* otherwise, if that property is not specified, the provider infers a schema
  from the object/relational mapping metadata (only) and drops the artifacts
  of the inferred schema.

+
The values `metadata-then-script` and `script-then-metadata` specify that a
combination of metadata and script is to be used and the order in which this
use occurs. If either of these values is specified and the resulting database
actions are not disjoint, the results are undefined and the dropping of
database artifacts may fail.

`jakarta.persistence.schema-generation.create-database-schemas` ::
This property specifies whether the persistence provider creates database
schemas in addition to creating database objects such as tables, sequences,
constraints, and so on. The value `true` specifies that the persistence
provider creates schemas in the database or generates DDL containing
`CREATE SCHEMA` commands. If this property is not supplied, the provider
must not create database schemas.footnote:[In Jakarta EE environments,
it is anticipated that the Jakarta EE platform provider may wish to control
the creation of database schemas rather than delegate this task to the
persistence provider. This property may also be specified in Java SE
environments.]

`jakarta.persistence.schema-generation.scripts.create-target`, ::
`jakarta.persistence.schema-generation.scripts.drop-target` ::
If scripts are generated, these properties must specify the target locations
where the scripts are written. Each property specifies either a `java.io.Writer`
configured for output of the DDL script or a string designating the file URL
of the DDL script. The properties should only be specified if scripts are
generated.
...
If scripts are generated by the persistence provider, and a connection to the
target database is not supplied, the `jakarta.persistence.database-product-name`
property must be specified. The value of this property should be the value
returned for the target database by the method `getDatabaseProductName` of the
JDBC `DatabaseMetaData` object. If sufficient database version information is
not included in the result of this method, the
`jakarta.persistence.database-major-version` and
`jakarta.persistence.database-minor-version` properties should be specified as
needed. These should specify the values returned by the `DatabaseMetaData`
methods `getDatabaseMajorVersion` and `getDatabaseMinorVersion` respectively.

`jakarta.persistence.schema-generation.create-script-source`, ::
`jakarta.persistence.schema-generation.drop-script-source` ::
These properties specify executable DDL scripts.footnote:[In Jakarta EE container
environments, it is generally expected that the container is responsible for
executing DDL scripts, although the container is permitted to delegate this task
to the persistence provider. If DDL scripts are executed in a Java SE environment,
or if the Jakarta EE container delegates the execution of scripts to the persistence
provider, these properties must be specified.] Each property specifies either a
`java.io.Reader` configured for reading the DDL script or a string designating a
file URL of the DDL script.

`jakarta.persistence.schema-generation.connection` ::
This property supplies a JDBC connection used for executing schema management
actions on the database.footnote:[This is intended for use in Jakarta EE environments,
where the platform provider may want to control the database privileges that are
available to the persistence provider. This connection is provided by the container
and should be closed by the container when the schema generation request or entity
manager factory creation completes. The connection provided must have credentials
sufficient for the persistence provider to carry out the requested actions.] If
this property is not specified, the persistence provider uses the `DataSource`
configured for use by the persistence unit to obtain a JDBC connection.

==== Data Loading [[a12918]]

Data loading via execution of a SQL DML script may occur as part of the
schema generation process, after the creation of the database artifacts,
or independently of schema creation. The specification of the property
`jakarta.persistence.sql-load-script-source` supplies the location of
the DML script which loads data, and helps control whether data loading
occurs.

`jakarta.persistence.sql-load-script-source` ::
This property specifies either a `java.io.Reader` configured for reading
a SQL DML script for data loading or a string designating a file URL where
the script is located.

When this property is set, the behavior depends on the schema
generation action specified by the property
`jakarta.persistence.schema-generation.database.action`:

- if the action is `create` or `drop-and-create`, the persistence provider
  must execute the DML script after creating the database artifacts,
- if the action is `populate`, the persistence provider must execute the
  DML script,
- if the action is `drop`, `validate`, or `none`, the persistence provider
  must not execute the DML script.

If `jakarta.persistence.sql-load-script-source` is set, but the property
`jakarta.persistence.schema-generation.database.action` is not set, the
behavior is undefined and is not portable between persistence providers.
...
`PersistenceUnitInfo` preserves the distinction between ordinary Java types,
package descriptors, and module descriptors. The `getManagedClassNames()`,
`getManagedPackageDescriptors()`, and `getManagedModuleDescriptors()` methods
report the entries explicitly declared by the corresponding `class`,
`package-descriptor`, and `module-descriptor` elements of `persistence.xml`.
The `getAllClassNames()`, `getAllPackageDescriptors()`, and
`getAllModuleDescriptors()` methods report the complete categorized
persistence-unit view obtained from explicit declarations, mapping metadata,
and container discovery, as applicable. None of these lists contains duplicates.

Only ordinary Java types are returned by `getAllClassNames()`. Package and
module descriptors are metadata sources and are not managed classes. Standard
or provider-specific class transformation and enhancement applies only to the
ordinary Java types returned by `getAllClassNames()` and never to package or
module descriptors.

The enum `PersistenceUnitTransactionType` lists options which determine
whether entity managers created by the factory are JTA or resource-local
entity managers.
...
The enum `SharedCacheMode` lists policies which control how the provider
determines if an entity type is eligible for storage in the second-level
cache, as defined in <<a3065>>. The `shared-cache-mode` element in
`persistence.xml` has no default value. The `getSharedCacheMode` method
must return `UNSPECIFIED` when the `shared-cache-mode` element is left
unspecified.
...
The enum `ValidationMode` lists the validation modes defined in <<a2374>>.
...
in Java SE environments. It may also be used for schema generation, that is,
to create database schemas and/or DDL scripts, as specified above in <<a12917>>.
...
==== Properties for Java SE Bootstrap

In a Java SE environment, any standard or vendor-specific property or hint used
during the creation of the `EntityManagerFactory` may be specified:

- by including it as an entry in a `Map` passed as the second argument to the
  `createEntityManagerFactory()` method of `Persistence`, or
- by calling the `property()` method of `PersistenceConfiguration`.

The following properties correspond to the elements and attributes in the
`persistence.xml` file. When one of these properties is specified via the
`Map` parameter of the `createEntityManagerFactory()` method, its value
overrides the value of any corresponding element or attribute specified in
the `persistence.xml` file for the named persistence unit, along with any
default value assigned by the persistence provider.
...
| `String`
| `validation-mode`
| Legal values are `auto`, `callback`, and `none`.
See <<a12382>> and <<a2374>>.
| `jakarta.persistence.defaultToOneFetchType`
| `String`
| `default-to-one-fetch-type`
| Legal values are `EAGER` and `LAZY`.
See <<fetching-policies>>
|===

The following properties correspond to properties in the `persistence.xml`
file. When one of these properties is specified via the `Map` parameter of
the `createEntityManagerFactory` method, its value overrides the value of
any corresponding `<property>` in the `persistence.xml` file for the named
persistence unit, along with any default value assigned by the persistence
provider.
...
persistence unit.
| `jakarta.persistence.jdbc.fetchSize`
| Override the default JDBC fetch size.
| `jakarta.persistence.jdbc.batchSize`
| Hint to enable JDBC statement batching by setting a batch size.
| `jakarta.persistence.validation.factory`
| Instance of `jakarta.validation.ValidatorFactory`.
| `jakarta.persistence.bean.manager`
| Instance of `jakarta.enterprise.inject.spi.BeanManager`.
|===
...
The `managedClass()`, `managedPackageDescriptor()`, and
`managedModuleDescriptor()` operations correspond to the `class`,
`package-descriptor`, and `module-descriptor` elements of `persistence.xml`,
respectively. A package is specified by its qualified package name without
the suffix `.package-info`, and a module is specified by its declared JPMS
module name, not `module-info`. Configuring a package or module descriptor does
not implicitly configure the ordinary Java types it contains.
...
=== Integration with CDI [[cdi]]

Entity listeners, defined in <<a2153>> and attribute converters,
defined in <<a2999>>, may be defined to make use of dependency
injection through the Contexts and Dependency Injection (CDI) API
<<a19500>> when CDI is availablefootnote:[CDI is available by
default in Jakarta EE environments, as required by the Jakarta EE
specification <<a19499>>.]. In this section, we refer to such objects
as _CDI-aware_.

A CDI-aware class that makes use of dependency injection may also
define lifecycle callback methods annotated with the `@PostConstruct`
and `@PreDestroy` annotations. These methods are invoked after injection
has taken place and before the attribute converter instance is destroyed,
respectively.

An instance of a CDI-aware class is a noncontextual object and does
not have a well-defined scope. It is not a CDI bean.

In a Jakarta EE environment, a persistence provider must support
dependency injection for CDI-aware classes. A persistence provider is
encouraged to also support dependency injection in other environments
where a CDI `BeanManager` is available, but that is not a requirement
of this specification. If CDI is not available, the persistence
provider must not instantiate a CDI-aware class.

<<a12802>> specifies how a persistence provider may obtain the
`BeanManager` in a Jakarta EE environment.

Where CDI-aware classes are supported, the persistence provider is
responsible for using the standard CDI SPI defined in <<a19500>> to:

- create an instance of the CDI-aware class, perform injection upon
  the instance, invoke its `@PostConstruct` and `@PreDestroy` methods,
  if any, and, later
- dispose of the instance when it is no longer of use.

When instantiating and injecting, the persistence provider must behave
as if it carries out the following steps:

1. Obtain a `BeanManager`.
2. Create an `AnnotatedType` instance for the CDI-aware class.
3. Create an `InjectionTarget` instance for the annotated type.
4. Create a `CreationalContext`.
5. Instantiate the CDI-aware class by calling the `produce()` method of
   `InjectionTarget`.
6. Inject the CDI-aware instance by calling the `inject()` method of
   `InjectionTarget` on the instance.
7. Invoke the `@PostConstruct` callback, if any, by calling the
   `postConstruct()` method of `InjectionTarget` on the instance.

When disposing of an instance, the persistence provider must behave as
if it carries out the following steps.

* Call the `preDestroy()` method of `InjectionTarget` on the instance.
* Call the `dispose()` method of `InjectionTarget` on the instance.
* Call the `release()` method of `CreationalContext`.

Persistence providers may optimize the steps above, for example, by
replacing calls to the standard CDI SPI with equivalent interactions
with vendor-specific interfaces, as long as the outcome is identical.
