...
or independently of schema generation. The specification of the property
`jakarta.persistence.sql-load-script-source` controls whether data
loading occurs.

`jakarta.persistence.sql-load-script-source` ::
The property `jakarta.persistence.sql-load-script-source` specifies either
a `java.io.Reader` configured for reading a SQL DML script for data loading
or a string designating a file URL where the script is located.
This property must be specified when a DML script is used for loading data
in a Java SE environment, or when the Jakarta EE container delegates the
execution of a DML script to the persistence provider.

In a Jakarta EE container environment, it is generally expected that the
container is responsible for executing DML scripts for data loading, but
the container is permitted to delegate this task to the persistence provider.
