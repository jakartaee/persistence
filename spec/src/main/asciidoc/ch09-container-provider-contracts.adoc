...
- In Jakarta EE environments these properties are passed by the container
  in the `Map` argument to either `PersistenceProvider.generateSchema()`
  or `createContainerEntityManagerFactory()`.
- In Java SE environments, they are passed in the `Map` argument to either
  the `Persistence.generateSchema()` or `createEntityManagerFactory()`.

In Jakarta EE environments, any strings identifying file URLs of script
sources or targets must specify absolute paths (not relative), and all
source and target file locations must be accessible to the application
...
file. When any of these properties are specified in the `Map` parameter passed
