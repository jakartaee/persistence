...
- In Jakarta EE environments these properties are passed by the container
  in the `Map` argument of either the `generateSchema()` method or the
  `createContainerEntityManagerFactory()` method of `PersistenceProvider`.
- In Java SE environments, they are passed in the `Map` argument of either
  the `generateSchema()` method or the `createEntityManagerFactory()` method
  of `Persistence`.
