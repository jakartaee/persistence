...
Deprecated `createEntityGraph(String)`, specified that the graph returned by `getEntityGraph()` is now mutable, and added `getEntityGraph(Class, String)`
...
Deprecate `name` member of `@MapKey` in favor of `value`.
