...
Added `Statement`, `CriteriaStatement`, `@NamedStatement` and `@NamedNativeStatement` to clearly distinguish statements which modify data from queries which return data; added `ofType()`, `withEntityGraph()`, and `asStatement()` to `Query`; added `StatementOrTypedQuery`
...
Added `@EntityListener` annotation

Allow `@EntityListeners` on a package descriptor or module descriptor
...
Added `augment()` methods to `CriteriaBuilder` for use in Jakarta Data implementations
...
==== Changes to Configuration and Bootstrap

Added `@PersistenceAgent` for injection of Jakarta EE container-managed `EntityAgent`

Allow named queries and statements to be defined on non-managed classes and interfaces
