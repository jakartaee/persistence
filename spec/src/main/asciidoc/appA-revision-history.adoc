...
Added `@StaticQuery`, `@StaticNativeQuery`, `@TypedQueryOptions`, `@StatementOptions` and `StaticStatementReference` for declaring static queries at the method level, along with additional members of `TypedQueryReference`

Added a new API for programmatic SQL result set mappings

Added `Statement`, `CriteriaStatement`, `@NamedStatement` and `@NamedNativeStatement` to clearly distinguish statements which modify data from queries which return data; added `ofType()`, `withEntityGraph()`, and `asStatement()` to `Query`
