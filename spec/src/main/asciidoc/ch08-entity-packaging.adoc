...
below in <<persistence-unit-definitions>>. Each `data-source` element declares
a named datasource, as specified below in <<data-source>>. The schema for the
`peristence.xml` file may be found below in <<persistence-xml-schema>>.

==== Data Source Definitions [[data-source]]
...
A `data-source` element occurring as a direct child of the root `persistence`
element of the `persistence.xml` file declares a named container-managed
`DataSource` bound to JNDI. This declaration is identical in semantics to the
`data-source` element of the Jakarta EE `application.xml` file.
...
`jta-data-source`, `non-jta-data-source`, data-source
...
A `data-source` element occurring as a child of a `persistence-unit` element
declares a `DataSource` managed by:
...
The subelements of `data-source` are defined above in <<data-source>>.
