...
unit is searched for classes bearing a discoverable annotation type.
Mapping metadata annotations found on these classes are processed;
where mapping annotations are missing, the classes are mapped using
mapping annotation defaults.
...
annotated class located in the root of the persistence unit is not
included in the persistence unit unless it is explicitly listed in
a `class` element of the `persistence.xml` file or in an XML mapping
file.
