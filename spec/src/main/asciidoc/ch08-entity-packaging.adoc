...
Furthermore, types declaring named queries and statements, as defined in
...
The set of types belonging to a persistence unit is specified using one or
...
unit is searched for:

- annotated managed persistence classes--any class with an `Entity`,
  `Embeddable`, or `MappedSuperclass` annotation, as defined in
  <<entities>>,
- annotated converter classes--any class with a `Converter` annotation,
  as defined in <<a2999>>, and
- classes declaring named queries and statements, as defined in
  <<named-queries-statements>>.
