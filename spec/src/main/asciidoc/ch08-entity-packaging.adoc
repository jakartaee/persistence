...
The `class` element identifies one ordinary Java type. It does not identify a
package descriptor or module descriptor.

The `package-descriptor` element identifies one package descriptor whose
...
The `module-descriptor` element identifies one module descriptor whose
...
In Java SE environments, an explicit list of all managed persistence class
names and any package or module descriptors whose annotations must be processed
...
may require that these sets are fully enumerated in each `persistence.xml` file.
