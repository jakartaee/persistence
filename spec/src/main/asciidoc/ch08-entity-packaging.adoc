...
A _persistence unit_ is a logical grouping that includes:

- an entity manager factory and its entity managers, together with
  their configuration information,
- the set of ordinary Java types, package descriptors, and module
  descriptors included in the persistence unit, including the managed
  classes managed by entity managers created by the entity manager factory,
- mapping metadata (in the form of metadata annotations and/or XML
  metadata) specifying the mapping of the managed classes to the database,
  and
- named queries and named statements involving these managed classes.
...
In Java SE environments, the metadata mapping files, jar files, and program
elements described in the following sections can be used. To ensure the
portability of a Java SE application, it is necessary to explicitly list the managed
persistence classes and any package or module descriptors included in the
persistence unit using the `class`, `package-descriptor`, and
`module-descriptor` elements of the
`persistence.xml` file. See <<a12305>>.
...
- package and module descriptors whose annotations belong to the
  persistence unit,
...
The `persistence-unit` element consists of:

- the `name` and `transaction-type` attributes and
- the following sub-elements:
  `description`, `provider`,
  `qualifier`, `scope`
  `jta-data-source`, `non-jta-data-source`,
  `mapping-file`, `jar-file`, `class`, `package-descriptor`,
  `module-descriptor`,
...
<persistence>
    <persistence-unit name="OrderManagement">
        ...
        <package-descriptor>com.widgets</package-descriptor>
        <module-descriptor>com.widgets.orders</module-descriptor>
    </persistence-unit>
</persistence>
----
...
The `transaction-type` attribute specifies whether entity managers and entity
agents created by the entity manager factory for the persistence unit are
JTA entity managers and entity agents or resource-local entity managers and
entity agents. The value of this element must be `JTA` or `RESOURCE_LOCAL`:
...
===== mapping-file, jar-file, class, package-descriptor, module-descriptor, exclude-unlisted-classes [[a12305]]
...
Furthermore, other discoverable types, as defined below in <<discoverable>>,
including types declaring named queries and statements, as defined in
<<named-queries-statements>>, must also be implicitly or explicitly denoted
as belonging to a persistence unit.

A given type may belong to more than one persistence unit.

An ordinary Java type, a package descriptor, and a module descriptor are
distinct kinds of program element. In this section, _type_ does not include
a package descriptor or module descriptor.

The sets of types, package descriptors, and module descriptors belonging to
a persistence unit are determined by:

- program elements bearing discoverable annotation types located in the root
  of the persistence unit (unless `<exclude-unlisted-classes/>` is specified);
- one or more object/relational mapping XML files;
- one or more JAR files to be searched for program elements; and
- explicit lists of types, package descriptors, and module descriptors.

Each resulting set is the union of its sources and contains no duplicates.
For any given managed class, mapping metadata annotations (or annotation
defaults) are overridden by XML mapping file information if there are both
annotations and XML mappings for that class. The minimum portable level of
overriding is at the level of the persistent field or property.

The types, descriptors, and jars listed as belonging to a persistence unit
must be on the classpath or module path as appropriate; referencing them from
the `persistence.xml` file does not cause them to be placed there.

All types and descriptors must be on the classpath or module path as
appropriate. This ensures that entity managers from different persistence
units that map the same class access the same identical class.

====== Discoverable annotation types [[discoverable]]

A _discoverable annotation type_ is an annotation type annotated with
the `Discoverable` meta-annotation.
...
This specification includes a number of discoverable annotation types,
including `@Entity`, `@Embeddable`, `@MappedSuperclass`, `@Converter`,
`@EntityListener`, `@NamedQuery`, and so on. A persistence provider may
define its own custom discoverable annotation types.

Exceptionally, the annotation `jakarta.data.repository.Repository`
must be treated as a discoverable annotation type, even though it is
not explicitly annotated `@Discoverable`.

====== Annotated Program Elements in the Root of the Persistence Unit

By default, in the Jakarta EE environment, the root of the persistence
unit is searched for ordinary Java types, package descriptors, and module
descriptors bearing discoverable annotation types. Each annotated program
element belongs to the corresponding categorized set of the persistence
unit. Mapping metadata annotations found on managed classes are processed;
where mapping annotations are missing, the classes are mapped using mapping
annotation defaults. Annotations found on package and module descriptors are
processed according to the semantics of those annotations.
...
annotated program element located in the root of the persistence unit does
not belong to the persistence unit unless it is explicitly listed in the
corresponding `class`, `package-descriptor`, or `module-descriptor` element of
the `persistence.xml` file, or, for an ordinary Java type, in an XML mapping
file.

In the Java SE environment, this behavior is not required. Portable
Java SE applications should explicitly list each ordinary Java type, package
descriptor, and module descriptor belonging to the persistence unit in the
corresponding `class`, `package-descriptor`, or `module-descriptor` element of
the `persistence.xml` file, or, for an ordinary Java type, in an XML mapping
file.
The `exclude-unlisted-classes` element is not intended for use in Java SE
environments.
...
An object/relational mapping XML file may add ordinary Java types to the
persistence unit, but it does not add package or module descriptors. In
particular, the `package` element of an `entity-mappings` document establishes
a default package for unqualified type names; it does not identify a package
...
elements. These JAR files are searched for ordinary Java types, package
descriptors, and module descriptors bearing discoverable annotation types.
Metadata annotations found on these program elements are processed by the
provider. Where mapping annotations are missing on a managed class, the class
is mapped using the mapping annotation defaults defined by this specification.

Such JAR files are specified by paths relative to the directory or jar
file that contains the root of the persistence unit.footnote:[Persistence
providers are encouraged to support this syntax in Java SE environments.]
...
====== Explicit Lists of Types and Descriptors

Lists of ordinary Java types, package descriptors, and module descriptors may
be specified instead of, or in addition to, the listed JAR files and mapping
files. Any metadata annotations found on these program elements are processed.
Where mapping annotations are missing on a managed class, the class is mapped
using the mapping annotation defaults.

<class/>:: The `class` element identifies one ordinary Java type. It does not identify a
package descriptor or module descriptor.

<package-descriptor/>:: The `package-descriptor` element identifies one package descriptor whose
annotations belong to the persistence unit. Its value is the qualified package
name, for example, `com.example.model`, and not
`com.example.model.package-info`. Naming a package descriptor does not cause
the ordinary Java types in that package to belong to the persistence unit.

<module-descriptor/>:: The `module-descriptor` element identifies one module descriptor whose
annotations belong to the persistence unit. Its value is the declared JPMS
module name and not `module-info`. Naming a module descriptor does not cause
the ordinary Java types or package descriptors in that module to belong to the
persistence unit.

In Java SE environments, an explicit list of all managed persistence classes
and any package or module descriptors whose annotations must be processed
must be specified to ensure portability. Portable Java SE applications should
not rely on the other mechanisms described here to determine the program
elements of a persistence unit. In Java SE environments, a persistence provider
may require that these lists are fully enumerated in each `persistence.xml` file.
...
The `default-to-one-fetch-type` element determines the default fetch
...
The `entity-listener` element declares an entity listener class with 
semantics equivalent to the `@EntityListener` annotation defined in 
<<entity-listener-annotation>>.

===== properties [[a12384]]

The `properties` element lists settings for both standard and
vendor-specific properties and hints that affect the configuration of
the persistence unit and creation of its `EntityManagerFactory`.

The recognized standard properties are listed below in <<persistence-xml-properties>>.

==== Persistence Unit Properties [[persistence-xml-properties]]

The following standard hints are intended for use in both Jakarta EE
and Java SE environments:

`jakarta.persistence.lock.timeout`::
Hint specifying a preferred pessimistic lock timeout in milliseconds.

`jakarta.persistence.query.timeout`::
Hint specifying a preferred query timeout in milliseconds.

The following properties are intended for use in Jakarta EE and
Java SE environments when an implementation of Jakarta Validation
is available:

`jakarta.persistence.validation.group.pre-persist`::
Jakarta Validation groups that are targeted for validation upon the
pre-persist event (overrides the default behavior).
`jakarta.persistence.validation.group.pre-merge`::
Jakarta Validation groups that are targeted for validation upon the
pre-merge event (overrides the default behavior).
...
Jakarta Validation groups that are targeted for validation upon the
pre-insert event (overrides the default behavior).
...
Jakarta Validation groups that are targeted for validation upon the
pre-upsert event (overrides the default behavior).
`jakarta.persistence.validation.group.pre-delete`::
Jakarta Validation groups that are targeted for validation upon the
pre-delete event (overrides the default behavior).

The following standard properties are intended for use in Java SE
environments.
...
The following properties are relevant in both SE and EE environments.
...
Scripts for use in schema generation, as defined in <<a12917>>, or for
bulk loading of data, defined in <<a12918>>, may be specified using the
following properties. These properties are intended for use in both
Jakarta EE and Java SE environments. Each property specifies the name of
a script packaged as part of the persistence application, or a file URL
identifying a script.

`jakarta.persistence.schema-generation.create-script-source`::
A DDL script which creates database objects.

`jakarta.persistence.schema-generation.drop-script-source`::
A DDL script which drops database objects.

`jakarta.persistence.sql-load-script-source`::
A DML script used for bulk data loading.
...
file URLs must be specified. In a Jakarta EE environment, such file URLs
must be absolute paths, and all source and target file locations must be
...
actions may also be controlled using the following properties in the
...
actions on the database. (See <<a12917>>.)
...
where the scripts are written. Each property specifies a string designating
the file URL of the DDL script. The properties should only be specified if
scripts are generated.

Ordinarily, the persistence provider automatically determines the database in
use by querying the JDBC database metadata or other vendor-specific heuristics.
In some environments, JDBC metadata is unavailable, and information about the
database must be explicitly specified.
...
In situations where JDBC metadata is not available, the property
`jakarta.persistence.database-product-name` must be specified. The value of
this property should be the value returned for the target database by the
method `getDatabaseProductName` of the JDBC `DatabaseMetaData` object. If
sufficient database version information is not included in the result of
this method, the `jakarta.persistence.database-major-version` and
`jakarta.persistence.database-minor-version` properties should be specified
as needed. These should specify the values returned by the `DatabaseMetaData`
...
==== Example Persistence Unit Configurations
...
providers. An entity manager factory supplying resource-local entity
...
- It is an error for a persistence unit defined at the level of an EAR
  to have the same name as a persistence unit defined by an EJB-JAR, WAR,
  or application JAR file contained within the EAR.footnote:[This was
  permitted prior to 4.0.]
