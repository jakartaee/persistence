...
[NOTE]
Terminology Note: In this context, the word "class" refers to compiled Java
`.class` files, and implicitly includes both Java types and Java module and
package descriptors.
...
The set of managed classes belonging to the persistence unit is the union
of these sources, with mapping metadata annotations (or annotation defaults)
for any given class being overridden by the XML mapping file information
...
The classes and jars listed as belonging to a persistence unit must be
...
In a Jakarta EE container, every Jakarta Data repository interface that
depends on a persistence unit is considered to belong to the persistence
unit, even if the repository interface is packaged in a separate archive.
...
unit is searched for classes bearing discoverable annotation types.
Each class annotated with a discoverable annotation type belongs to
the persistence unit. Mapping metadata annotations found on these
classes are processed; where mapping annotations are missing, the
classes are mapped using mapping annotation defaults.
...
annotated class located in the root of the persistence unit does not
belong to the persistence unit unless it is explicitly listed in a
`class` element of the `persistence.xml` file or in an XML mapping
file.

In the Java SE environment, this behavior is not required. Portable
Java SE applications should explicitly list each class belonging to
the persistence unit in a `class` element of the `persistence.xml`
file or in an XML mapping file. The `exclude-unlisted-classes` element
is not intended for use in Java SE environments.
...
elements. These JAR files are searched for classes bearing discoverable
annotation types, and metadata annotations found on these classes are
processed by the provider. Where mapping annotations are missing, the
classes are mapped using the mapping annotation defaults defined by this
specification.
...
A list of classes may be specified instead of, or in addition to, the
listed JAR files and mapping files. Any mapping metadata annotations
found on these classes are processed. Where mapping annotations are
missing, the classes are mapped using the mapping annotation defaults.
The `class` element is used to list a class.
