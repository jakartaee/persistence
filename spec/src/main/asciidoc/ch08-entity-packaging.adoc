...
Ordinarily, the persistence provider automatically determines the database in use by querying the JDBC database metadata or other vendor-specific heuristics.
In some environments, JDBC metadata is unavailable, and information about the database must be explicitly specified.
...
In situations where JDBC metadata is not available, the `jakarta.persistence.database-product-name` property must be
