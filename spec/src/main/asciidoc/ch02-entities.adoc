...
When an element collection has declared type `Map<K,V>`, the type
argument `A` is taken, by default, to be its key type. The key type
...
any XML descriptor, the following defaulting rules are applied in order:
...
* A unidirectional or bidirectional one-to-one relationship,
  ...
  many-to-one relationships may be mapped to a join table. The `JoinTable`
