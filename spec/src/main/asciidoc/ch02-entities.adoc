...
An attribute of an entity must not have with mapping annotations
...
identity. Such modifications should never alter the _persistent_
...
- A basic type, an embeddable class, or any collection type is
  ...
  hold references to distinct instances of the value type when the
  entity is reloaded from the database.
...
member must be explicitly specified.
...
or an entity type. The key type and map type are completely
...
An attribute which represents one side of such an association must
...
declares as a `mapped-superclass` in its XML descriptor.
...
Persistent relationships involving by a mapped superclass must be unidirectional.
...
a table or column name when the name of differs only in case from the
