...
When a non-entity class is annotated with an annotation type defined by this
specification, the behavior is undefined. The persistence provider might simply
ignore such annotations.

A non-entity class cannot be passed as an argument to a method of the `EntityManager`, `EntityAgent`, or `Query` interfacefootnote:[This includes
