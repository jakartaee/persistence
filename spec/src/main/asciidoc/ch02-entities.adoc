...
represents the _persistent identity_ of the instance.
...
<<a1955>>. If the inverse side of an association is modified without
...
==== Cascade Remove and Orphan Removal

The `cascade` member of a relationship modeling annotation specifies
a list of persistence operations which should be cascaded to the
associated entity, as specified below in <<a1929>>. An attribute
annotated `ManyToOne` or `ManyToMany` must not specify `REMOVE`
among its cascaded operation types. An application which specifies
`cascade=REMOVE` for a `ManyToOne` or `ManyToMany` associations is
not portable.

The `orphanRemoval` member of the `OneToOne` and `OneToMany`
controls the enablement of _orphan removal_; `orphanRemoval=true`
activates the following behaviors:
...
The `orphanRemoval` functionality is intended for child entities that are
privately owned by their parent entity. Portable applications must not depend
on a specific timing of the removal operation and must not reassign an entity
that has been orphaned to another relationship, nor otherwise attempt to make it
persistent, reversing the effect of orphan removal.
