...
- When `Access(FIELD)` is specified at the class level, an individual
  ...
  access by annotating a property getter `Access(PROPERTY)`. If a mapping
  annotation is placed on a property getter which is not annotated
...
property access. However, the persistence provider must report an error if:
