...
the same column if one of the persistent attributes is declared
with `insertable=false, updatable=false`, as defined later in
<<metadata-annotations>>. If two distinct attributes map the same
column, and neither is mapped `insertable=false, updatable=false`,
the behavior is undefined.
