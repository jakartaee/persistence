...
==== Duplicate Column Mappings in Inheritance Hierarchies

With an important exception noted below:

- All persistent attributes declared by a given entity class must
  map to distinct columns. This restriction is automatically
  satisfied by default, due to the restriction specified in above
  in <<dupe-attributes>>. It can be violated if two `@Column` or
  `@JoinColumn` annotations specify the same column name. It is
  the responsibility of the application developer to avoid such
  mappings.

- In addition, when the single table inheritance mapping strategy
  defined in <<single-table>> is used to map an entity class
  inheritance hierarchy, all persistent attributes defined across
  the whole hierarchy must map to distinct columns. It is the
  responsibility of the application developer to satisfy this
  restriction by explicitly specifying distinct column mappings
  for persistent attributes with identical names.

- Similarly, when the table-per-concrete class inheritance mapping
  strategy defined in <<table-per-concrete-class>> is used to map
  an entity class inheritance hierarchy, all persistent attributes
  declared or inherited by a given entity class must map to distinct
  columns. It is the responsibility of the application developer to
  satisfy this restriction by explicitly specifying distinct column
  mappings for persistent attributes with identical names.
...
As an exception, two persistent attributes (declared by the same
entity class or by distinct entity classes) are permitted to map
the same column if:
...
If two distinct persistent attributes declared or inherited by
a given entity class map the same column and neither is mapped
`insertable=false, updatable=false`, the behavior is undefined.
Similarly, if two persistent attributes are declared within the
same entity class hierarchy with distinct types and map the same
column, the behavior is undefined.
