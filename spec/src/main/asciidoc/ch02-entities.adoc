...
`transient`, or `@Transient`, such a field or property is not persistent.
...
A null value assigned to a collection-valued persistent field or property
is interpreted as an empty collection. The persistence provider runtime
is permitted to replace a null value held by a collection-valued field or
property of an entity with an empty collection.

==== Persistent Attribute Type [[attribute-categories]]

Every persistent field or property of an entity is either:

- an attribute of **basic type**, as defined in <<a486>>, or of a type
  converted to a basic type through use of a _converter_, as defined
  in <<a2999>>,
- an **embedded attribute** whose type is a fine-grained embeddable
  class, as defined <<a487>>,
- an **element collection**, that is, a collection whose elements are
  instances of a basic type or embeddable class, as defined in <<a494>>,
- a **relationship** (or **association**) with another entity type, as
  defined in <<a516>>, which is further classified by its multiplicity:
  one-to-one, many-to-one, one-to-many, or many-to-many.

The enumeration
`jakarta.persistence.metamodel.Attribute.PersistentAttributeType`
defines a categorization of persistent entity attributes:

- `BASIC` for basic attributes,
- `EMBEDDED` for embedded attributes,
- `ELEMENT_COLLECTION` for element collections, and
- `MANY_TO_ONE`, `ONE_TO_ONE`, `ONE_TO_MANY`, and `MANY_TO_MANY` for
  associations of the indicated multiplicity.

Each persistent attribute of an entity belongs to exactly one of the
listed types.

- An attribute annotated `@Basic` is a basic attribute.
- An attribute annotated `@Embedded` is an embedded attribute.
- An attribute annotated `@ElementCollection` is an element collection.
- An attribute annotated `@ManyToOne`, `@OneToOne`, `@OneToMany`, or
  `@ManyToMany` is an association of the indicated multiplicity.

If none of the above annotations occur on a persistent attribute,
the defaulting rules specified below in <<a511>> determine which
category it belongs to.

An attribute of an entity must not have mapping annotations
indicating conflicting persistent attribute types. For example, a
field must not be annotated `@Basic @Embedded`,
...
==== Value Types and Reference Types [[value-type-reference-type]]

In the Java language, an instance of a non-primitive type not
explicitly declared as a `value` type has a well-defined object
identity unique to the instance and is passed _by reference_,
meaning that assignment of the instance to a field, local variable,
or method parameter, followed by later access to the assigned value,
preserves the identity of the instance.

In general, operations performed by a persistence provider are not
required to preserve the object identity of a value stored in an
attribute of an entity. The provider is permitted to modify the
value held in the attribute to a new value with a distinct object
identity. But such modifications never alter the _persistent_
representation of the entity because the Java object identity of
a value held in an attribute is never part of the persistent
state of the entity.

In particular:

- A basic type, an embeddable class, or any collection type is
  considered a _value type_, and the provider is not required
  to respect the integrity of shared references to an instance
  of such a type. If multiple fields referenced the same instance
  of a value type when an entity was made persistent, those fields
  may hold references to distinct instances of the value type when 
  the entity is reloaded from the database.
- An entity type is considered a _reference type_, and, instead of
  object identity, every managed instance of an entity type has a
  well-defined _persistent identity_. The persistence provider must
  respect its persistent identity by:

  * preserving the persistent identity of an instance when it is
    made persistent and then later reloaded from the database,
    and
  * ensuring that, within a single persistence context, at most
    one object represents the state of each distinct persistent
    identity, thus preserving the integrity of shared references
    to entities belonging to the given persistence context.

+
This requirement notwithstanding, the container is permitted to
introduce an indirection or _proxy_ between the object which
represents the state of an entity and attributes which hold
references to the entity. Thus, object identity might still not
be respected in a strict sense, not even for a reference type.
...
of the `EntityManager` or `EntityAgent`, as described in <<a1060>>, and
represents the _persistent identity_ of the instance, a concept which is
defined more carefully later in <<managed-entities>>.
...
[NOTE]
Terminology Note: In the community, the primary key of an entity is usually
called its _identifier_, but for historical reasons that term is not used
...
- the `MapsId` annotation, as defined below in <<mapsid-derived-identity>>,
  if some other attribute or attributes annotated `Id` or `EmbeddedId`
...
===== Use of `MapsId` to Represent Derived Identity [[mapsid-derived-identity]]

The `MapsId` annotation defined in <<mapsid>> allows a relationship to the
parent entity of an entity with a derived identity to be redundantly
represented by:

1. a relationship attribute annotated  `ManyToOne` or `OneToOne`, and
2. a simple or composite primary key field or property, as defined above in
   <<a132>>.

As specified in <<a14687>>, an embedded id class cannot declare a field or
property representing a relationship. Therefore, a `ManyToOne` or `OneToOne`
relationship mapping the primary key column or columns of an entity with an
embedded id must be declared using `MapsId`. On the other hand, a `ManyToOne`
or `OneToOne` relationship mapping the primary key column or columns of an
entity with an id class may be declared with either `Id` or `MapsId`, as
specified below in <<a155>>.

When `MapsId` is used to map a `ManyToOne` or `OneToOne` relationship, the
entity class, id class, or embedded id class must have a corresponding field
or property with the same type as the identifier of the associated entity.
The `value` member of the `MapsId` annotation identifies this field or property,
which is said to be _mapped by_ the relationship attribute annotated `MapsId`.

- If the dependent entity has the same primary key type as the parent entity,
  or if the dependent entity represents its composite primary key as an id
  class, the field or property mapped by the relationship attribute must be
  declared using the `Id` or `EmbeddedId` annotation in the entity class. If
  the associated entity has a simple primary key, it is declared using the `Id`
  annotation; otherwise, if the associated entity has a composite primary key,
  it must be declared using the `EmbeddedId` annotation.

- If the dependent entity represents its composite primary key as an embedded
  id, no `Id` or `EmbeddedId` annotation is required on the field or property
  mapped by the relationship attribute.

===== Specification of Derived Identities [[a155]]

A dependent entity may have a composite primary key represented as an id class
or embedded id, or it may have a simple primary key.

====== Derived Identity Represented as Id Class
...
- The names and types of the attributes of the id class and the `Id`
  and `EmbeddedId` attributes of the dependent entity class must
  correspond as follows:

* The `Id` or and `EmbeddedId` attribute of the dependent entity class
  and the corresponding attribute in the id class must have the same
  name.
...
* An `Id` attribute of the dependent entity class which is mapped by a
  relationship to the parent entity annotated `MapsId` must be of the
  same Java type as the `Id` attribute of the parent entity. The `value`
  element of the `MapsId` annotation of the relationship attribute must
  specify the name of the `Id` attribute.

* An `EmbeddedId` attribute of the dependent entity class which is mapped
  by a relationship to the parent entity annotated `MapsId` must be of the
  same Java type as the id class or embedded id of the parent entity. The
  `value` element of the `MapsId` annotation of the relationship attribute
  must specify the name of the `EmbeddedId` attribute.
...
====== Derived Identity Represented as Embedded Id
...
- Otherwise, the `value` element of the `MapsId` annotation must specify
  the name of the attribute within the embedded id which is mapped by the
  annotated relationship attribute and this attribute of the embedded id
  must be of the same type as the primary key of the parent entity.
...
====== Derived Identity Represented as Simple Primary Key
...
- If the dependent entity uses an id class, the `Column` annotation may be
  used to override the default mapping of `Id` attributes which are _not_
...
public class PaycheckId {
    int period; // matches name of @Id attribute
...
SELECT d
FROM Paycheck d
WHERE d.period = 1 AND d.emp.empName = 'Sam'
----

*Case (b):* The dependent entity uses `IdClass` and `MapsId` to represent a composite key:

[source,java]
----
public class PaycheckId {
    int period; // matches name of @Id attribute
    long empId; // matches name of @Id attribute and type of Employee PK
}
...
*Case(c):* The dependent entity uses `EmbeddedId` to represent a composite key:
...
SELECT d
FROM Paycheck d
WHERE d.id.period = 1 AND d.emp.empName = 'Sam'
----
...
----
public class PaycheckId {
    int period; // matches name of attribute
...
SELECT d
FROM Paycheck d
WHERE d.period = 1 AND d.emp.firstName = 'Sam'
----

*Case (b):* The dependent entity uses `IdClass` and `MapsId`:

[source,java]
----
public class PaycheckId {
    int period; // matches name of attribute
    EmployeeId empId; //matches name of attribute and type of Employee PK
}
...
SELECT d
FROM Paycheck d
WHERE d.period = 1 AND d.empId.firstName = 'Sam'
----

*Case (c):* The dependent entity uses
...
SELECT d
FROM Paycheck d
WHERE d.id.period = 1 AND d.emp.firstName = 'Sam'
----
...
SELECT d
FROM Paycheck d
WHERE d.id.period = 1 AND d.id.empPK.firstName = 'Sam'
----
...
public class Paycheck {
    @Id
    @Column(name="pay_period") // default column name is overridden
    int period;
...
SELECT d
FROM Paycheck d
WHERE d.period = 1 and d.emp.empId.firstName = 'Sam'
----
...
public class PaycheckId {
    int period; // matches name of @Id attribute
    EmployeeId empId; // matches name of @Id attribute and type of embedded id of Employee
}
...
*Case (c):* The dependent entity uses `EmbeddedId`:
...
SELECT d
FROM Paycheck d
WHERE d.id.period = 1 and d.emp.empId.firstName = 'Sam'
----
...
SELECT m
FROM MedicalHistory m
WHERE m.personId.firstName = 'Charles'
----

*Case (c):* The dependent entity uses the
...
is not shareable across persistent entity instances; sharing of an
embedded object across multiple managed entity instances has undefined
semantics. As described above in <<value-type-reference-type>>, the
persistence provider is not required to respect the object identity of
a value type, and this identity is typically not made persistent in
the database.

Thus, an embedded object may be an instance of a Java value type.
...
- is not annotated as `Entity`;
- must not be `abstract`;
- but may be a `value` type.
...
An element collection must have declared type `java.util.Collection`,
`java.util.Set`, `java.util.List`, or `java.util.Map`. Collections of
type `Map` are special, and are described below in <<map-collections>>.

When an element collection has declared type `Collection<E>`, `Set<E>`,
or `List<E>`, the type argument `E` is taken, by default, to be its
element type. The element type may be explicitly specified using the
`targetClass` member of the `ElementCollection` annotation. If an
element collection is declared with a raw type, the `targetClass`
member must be explicitly specified. Otherwise, if the element
collection is not declared with a raw type, the `targetClass` is
optional, and it must specify an entity type which is assignable 
to the type argument `E`.

An element collection maps to a _collection table_, as defined in
<<a14250>>.

If an embeddable class, including an embeddable class embedded
within another embeddable class, is contained, directly or
indirectly, within an element collection:

- it must not have its own element collection;
- it must not have a one-to-many or many-to-many relationship to
  an entity;
...
An element collection may be of type `java.util.Map`. Such a map
collection has two different element types:

- a _key type_ and
- a _value type_.

A key or value type may be a basic type, an embeddable class,
or an entity type. The key type and value type are completely
...
When an element collection has declared type `Map<K,V>`, the type
argument `V` is taken, by default, to be its value type. The value
type may be explicitly specified using the `targetClass` member of
the `ElementCollection` annotation. If an element collection is
declared with a raw type, the `targetClass` member must be explicitly
specified.

When the value type of the map is a basic type or an embeddable class,
the map maps to a collection table. The default column mappings for the
map value are derived according to the default mapping rules for the
`CollectionTable` annotation, as defined in <<a14250>>.

- If the map value type is a basic type, the `Column` annotation is
  used to override the default column mappings.

- If the map value type is an embeddable class, the `AttributeOverride`
  and `AssociationOverride` annotations are used to override the default
  column mappings, as described in <<a14084>> and <<a13942>>.

When the value type of a map is an entity type:

- the map maps to a join table if the map represents a many-to-many
  relationship or, by default, if it represents a unidirectional
  one-to-many relationship, but
- if the map represents the collection-valued side of a bidirectional
  one-to-many/many-to-one relationship, then, by default, the map maps
  to the primary table of the value entity type.

In this case, default mappings are described in <<a538>>.

==== Map Keys

When an element collection has declared type `Map<K,V>`, the type
argument `K` is taken, by default, to be its key type. The key type
may be explicitly specified using the `MapKeyClass` annotation. If
an element collection is declared with a raw type, the `MapKeyClass`
annotation is required.

When the map value type is an entity type, the `MapKey` annotation
specifies that the map key is itself the primary key or a persistent
field or property of the map value entity.

The `MapKey` and `MapKeyClass` annotations are mutually exclusive;
an element collection may have either a `@MapKey` or `@MapKeyClass`
annotation, or neither, but not both.

Dedicated annotations are provided for specifying column mappings
for map keys:

- If the map key type is a basic type, the `MapKeyColumn` annotation
  is used to specify the column mapping for the map key. If the
  `MapKeyColumn` annotation is not specified, the default values of
  the `MapKeyColumn` annotation apply as described in <<a15367>>.

- If the map key type is an embeddable class, the mappings for the
  map key columns are defaulted according to the default column
  mappings for the embeddable class. (See <<a14330>>). The
  `AttributeOverride` annotation can be used to override these
  default mappings, as described in <<a14084>>.

- If the map key type is an entity type, the `MapKeyJoinColumn`
  and `MapKeyJoinColumns` annotations are used to specify its
  column mappings. If the primary key of the referenced entity
  is a simple primary key and the `MapKeyJoinColumn` annotation
  is not specified, the default values of the `MapKeyJoinColumn`
  annotation apply as specified in <<a15450>>.

When the map key type is an embeddable class, the embeddable class
must implement the `hashCode` and `equals` methods consistently
with the semantics of equality for the database columns to which
the embeddable is mappedfootnote:[Note that when an embeddable
instance is used as a map key, these attributes represent its
identity. Modification of an embeddable instance used as a map key
has undefined semantics and should be avoided.].

=== Mapping Defaults for Non-Relationship Fields or Properties [[a511]]

If a persistent field or property other than a relationship property
is _not_ annotated with one of the mapping annotations listed above in
<<attribute-categories>>, and if no equivalent mapping information is
supplied in any XML descriptor, nor via any provider-specific mechanism,
the following defaulting rules are applied in order:

1. If the type of the field or property is a class annotated with the
   `Embeddable` annotation, the field or property is treated as if it
   were annotated with the `Embedded` annotation, as defined in <<a14634>>
   and <<a14672>>.
2. If an attribute converter, as defined in <<a2999>>, is applied to the
   field or property, either via use of the `Convert` annotation, as
   specified in <<convert>>, or because the converter is automatically
   applied, as defined in <<auto-apply>>, the field or property is treated
   as if it were annotated as `Basic`.
3. Otherwise, if the type of the field or property is one of the basic
   types listed in <<a486>>, it is treated as if it were annotated as
   `Basic`, as defined in <<a14205>>, <<a14719>>, <<a15087>>, and <<a16361>>.

If no annotation is present and neither of the above rules is applicable,
the behavior is undefined. A persistence provider might report an error
when the persistence unit is initialized.

=== Entity Relationships [[a516]]

Every relationship (or association) between two entity types has
well-defined _multiplicity_, either:

- **one-to-one**,
- **one-to-many** / **many-to-one** (depending on which side the relationship
  is viewed from), or
- **many-to-many**.

Every attribute representing one side of such an association must
be annotated with a _relationship modeling annotation_:

- `OneToOne` if the association has one-to-one multiplicity--in
  which case the declared type of the attribute must be an entity
  type,
- `ManyToOne` if the association has many-to-one multiplicity
  and the declared type of the attribute is an entity type--that
  is, if it is the many-valued (child) side of the association,
- `OneToMany` if the association has one-to-many multiplicity
  and the declared type of the attribute is a collection type--that
  is, if it is the single-valued (parent) side of the association, or
- `ManyToMany` if the association has many-to-many multiplicity--in
  which case the declared type of the attribute must be a collection
  type.

[NOTE]
These annotations mirror common practice in relational database schema
modeling. The use of the relationship modeling annotations allows the
object/relationship mapping of associations to the relational database
schema to be fully defaulted, to provide an ease-of-development facility.
This is described in <<a538>>.

For an attribute annotated `OneToOne` or `ManyToOne`, the declared
type of the attribute is taken to be the associated entity type. If
the declared type is not an entity type, the `targetClass` member
must be explicitly specified, or this information must be supplied
in the XML descriptor.

For an attribute annotated `OneToMany` or `ManyToMany`, the type of
the attribute is a collection type.

- If the attribute has declared type `Collection<E>`, `Set<E>`, or
  `List<E>`, the associated entity type is taken to be the type
  argument `E`.
- If the attribute has declared type `Map<K,V>`, the associated
  entity type is taken to be the second type argument `V`.

If the inferred type `E` or `V` is not an entity type, or if the
declared type of the attribute is a raw type, the `targetClass`
member must be explicitly specified, or this information must be
supplied in the XML descriptor.

Relationships are polymorphic. A relationship to an entity which has
entity subclasses may be populated with an instance or instances of
the subclass.

A relationship may be _bidirectional_ or _unidirectional_.

- Every bidirectional relationship has both an _owning_ side and an
  _inverse_ (or _non-owning_, or _unowned_) side.
- A unidirectional relationship has only an owning side.

Modifications to the owning side of a relationship determine the
updates made to the relationship in the database, as described in
<<flush>>. If the inverse side of an association is modified without
a corresponding modification to the owning side, the behavior is
undefined. The persistence provider is permitted to ignore any
modification made only to the inverse side of a bidirectional
association.

The following rules apply to bidirectional relationships:

The inverse side of a bidirectional relationship must identify the
owning side by specifying the `mappedBy` element of the `OneToOne`,
`OneToMany`, or `ManyToMany` annotation. The `mappedBy` element
specifies the name of the property or field of the associated entity
that represents the owning side of the relationship.

* The many-valued side of one-to-many / many-to-one bidirectional
  relationships must be the owning side; therefore, the `ManyToOne`
  annotation does not have a `mappedBy` element.
* The owning side of a one-to-one bidirectional relationship is
  declared by the entity mapped to the table that contains the
  corresponding foreign key column, unless the relationship
  maps to a join table, in which case either side may be declared
  as the owning side.
* A many-to-many bidirectional relationship always maps to a join
  table, and so either side may be declared as the owning side.

This specification recognizes two basic strategies for mapping an association:

- In a _foreign key mapping_, the relationship is mapped to a foreign key
  column in a (primary or secondary) table of one of the participating
  entities, known as the _child entity_.
- In a _join table mapping_, the relationship is mapped to a dedicated
  intermediate join table that represents the association itself.

<<a538>> specifies default mappings for entity relationships. These defaulted
mappings may be customized through the use of mapping annotations or via an XML
descriptor. Furthermore, this specification requires support for the following
additional mapping strategies:

* A unidirectional one-to-many relationship may be mapped to a foreign key
  column of the primary table of the target entity. The `JoinColumn`
  annotation found in <<a14922>> or equivalent XML element must be used to
  indicate this mapping strategy.

* A unidirectional or bidirectional one-to-one relationship,
  bidirectional many-to-one/one-to-many relationship, or unidirectional
  many-to-one relationship may be mapped to a join table. The `JoinTable`
  annotation found in <<a15022>> or corresponding XML element must be used
  to indicate this mapping strategy.

Annotations which customize the mapping of a relationship must be specified
on the owning side of a bidirectional relationship. If a mapping annotation
is specified on the inverse side of a bidirectional relationship, the
behavior is undefined. A persistence provider might produce an error when
the persistence unit is initialized.

Any overriding of mapping defaults must be consistent with the multiplicity
implied by relationship modeling annotation. For example, a `ManyToMany`
association must not be mapped to a join table with a unique key constraint
on a foreign key.

The persistence provider handles the object/relational mapping of a
relationship, including their persistence in the database, according to the
metadata provided along with the entity class and the referential integrity
constraints existing in the database (e.g., foreign key constraints).

[NOTE]
====
The application bears responsibility for maintaining the consistency of
relationships--for example, the application must ensure that the “one” and
“many” sides of a bidirectional relationship are consistent with one another
when the application modifies either side at runtime.
====

When a `OneToMany` or `ManyToMany` relationship has no associated entities,
the persistence provider must populate the attribute with an empty collection
when loading an entity from the database.

The `cascade` member of a relationship modeling annotation specifies a list
of persistence operations which should be cascaded to the associated entity,
as specified later in <<a1929>>. An attribute annotated `ManyToOne` or
`ManyToMany` must not specify `REMOVE` among its cascaded operation types.
An application which specifies `cascade=REMOVE` for a `ManyToOne` or
`ManyToMany` association is not portable.

=== Relationship Mapping Defaults [[a538]]

This section defines the mapping defaults that apply to entity associations
declared using the relationship modeling annotations defined above in <<a516>>.
These mapping defaults also apply when the XML descriptor is used to declare
a relationship.
...
An entity class might inherit another entity class. If this is the case,
then:

- any association targeting the inherited entity class is by nature a
  polymorphic association, and may hold a reference to an instance of
  the inheriting entity class, and
- any Jakarta Persistence query against the inherited entity class is
  also polymorphic and may return an instance of the inheriting entity
  class.

An abstract class may be annotated `Entity` or declared as an `entity`
in its XML descriptor. An abstract entity class may participate in a
relationship, and may occur in the `from` clause of a query. Abstract
entity classes are discussed below in <<abstract-entity-classes>>.

An entity might extend a non-entity class, and a non-entity class might
extend an entity class. Such a non-entity class might be declared as a
mapped superclass defining common persistent state and object/relational
mapping information inherited by its entity subclasses, as discussed
below in <<mapped-superclasses>>, but, as described below in
<<non-entity-classes>>, it is not required to be. If a non-entity class
defines persistent state, it must be declared as a mapped superclass.

==== Abstract Entity Classes [[abstract-entity-classes]]

An abstract entity differs from a concrete entity only in that it cannot
be directly instantiated. This specification defines no special restrictions
applying only to abstract entity classes.

The following example shows the use of an abstract entity class in an entity
...
An entity might inherit a superclass that declares persistent entity state and
mapping information, but which is not itself an entity. Typically, the purpose
of such a _mapped superclass_ is to define state and mapping information that
is common across multiple entity classes but which maps to disparate, unrelated
database tables.

A mapped superclass must be annotated with the `MappedSuperclass` annotation or
declared as a `mapped-superclass` in its XML descriptor.

A mapped superclass might be an abstract class or a concrete class.

A mapped superclass, unlike an entity, is not queryable and must not be passed
as an argument to `EntityManager`, `EntityAgent`, or `Query` operations.

Persistent relationships involving a mapped superclass must be unidirectional.

- A mapped superclass may declare a unidirectional association to an entity
  class, but
- the mapped superclass must not be the target of an association declared by
  any other class.

A mapped superclass never maps to a database table. The persistent state
inherited by an entity from a mapped superclass maps to a table determined
by the entity itself, as if the attributes of the mapped superclass were
declared by the entity which most directly inherits the mapped superclass.

The persistent attributes of a mapped superclass may be mapped in the same
way as the attributes of an entity class. Such mappings are meaningful only
in the context of an entity subclass of the mapped superclass, since no table
exists for the mapped superclass itself. From the point of view of such an
entity subclass, the inherited mappings are interpreted in the context of the
tables mapped by the subclass. Mapping information inherited from a mapped
superclass can be overridden in such subclasses using the `AttributeOverride`
and `AssociationOverride` annotations or corresponding XML elements.

All other entity mapping defaults apply equally to a mapped superclass.

The following example illustrates the use of a concrete class as a mapped
superclass.
...
==== Non-Entity Classes in the Entity Inheritance Hierarchy [[non-entity-classes]]

An entity might inherit a non-entity superclass, which might be concrete
or abstract.footnote:[The superclass must not be an embeddable class or
id class.]

The non-entity superclass serves for inheritance of behavior only. The
state of a non-entity superclass is not persistent. Any state inherited
from a non-entity superclass is treated as if it were declared `@Transient`
by the inheriting entity class. This non-persistent state is not managed by
the entity managerfootnote:[If a transaction-scoped persistence context is
used, it is not required to be retained across transactions.].

When a non-entity class is annotated with an annotation type defined by
this specification, the behavior is undefined. The persistence provider
might simply ignore such annotations.

A non-entity type may not be used as an entity type, even if it inherits
an entity type:

- A non-entity class cannot be passed as an argument to a parameter of a
  method of `EntityManager` or `EntityAgent` which expects an entity type.
- If `object.getClass()` return a non-entity class, then `object` cannot
  be passed as an argument to a parameter of a method that expects an
  instance of an entity type, even if `object` is assignable to an entity
  type.
- A non-entity type may not be used as the root entity or joined entity
  type in a query written in JPQL.

The following example illustrates the use of a non-entity class as a superclass
of an entity class.
...
The mapping of entity class hierarchies to database tables is specified
through metadata.
...
* With a **single table per class hierarchy**, every field declared by
  any class in the hierarchy maps to a column of the same table.
  A subclass shares the table of its superclasses.
* With a **table per concrete entity class**, every field declared or
...
* For the **joined subclass** strategy, any field which is declared by
...
These mapping strategies are enumerated by `InheritanceType`, and the
mapping strategy for an entity class hierarchy may be specified by the
`@Inheritance` annotation or `inheritance` element of an XML descriptor,
as defined by <<a14891>>. The inheritance strategy for the whole entity
class hierarchy is determined by the mapping of the root class of the
hierarchy. If the `@Inheritance` annotation is used, it must be placed
on the root class.

An implementation of this specification is required to support the single
table per class hierarchy inheritance mapping strategy and the joined
subclass strategy.
...
Support for the combination of inheritance strategies within a single
entity inheritance hierarchy is not required by this specification.

==== Single Table per Class Hierarchy Strategy [[single-table]]

In this strategy, all classes in a hierarchy map to the same table.
The table has a column serving as a _discriminator column_, that is,
a column whose value identifies the specific subclass to which the
entity instance that is represented by a row of the table belongs.

This mapping strategy provides good support for polymorphic relationships
between entities and for queries that range over the class hierarchy.

Its drawback is that it requires that columns mapped by attributes of
...
- The root of the class hierarchy is represented by a table with
  columns mapped by attributes declared by the root class.
- Each subclass is represented by a separate table containing
  only those columns that are mapped by attributes declared by that
  subclass (and not inherited from its superclass), along with the
  column or columns holding its primary key. The primary key of the
  subclass table doubles as a foreign key of the superclass table.

This strategy provides good support for polymorphic relationships
between entities and is very well normalized.

Its drawback is that it requires at least one `JOIN` operation to
retrieve instances of a subclass. In a very deep class hierarchy,
this might lead to poor performance. Queries that range over the
class hierarchy likewise require joins.

==== Table per Concrete Class Strategy [[table-per-concrete-class]]

In this mapping strategy, each concrete class in the hierarchy is
mapped to a completely separate table. Every attribute of the class,
including inherited attributes, maps to a column or columns of the
table belonging to the concrete class.

This strategy has the following drawbacks:

* It provides very poor support for polymorphic relationships.
* It requires the use of a SQL `UNION` query (or multiple SQL queries)
  when a query ranges over the class hierarchy.

This inheritance mapping strategy is not required in the current
release of this specification.

==== Duplicate Attributes in Inheritance Hierarchies [[dupe-attributes]]

An entity class must not have two persistent attributes--declared
or inherited--with the same name. If an entity class or mapped
superclass declares a persistent attribute with the same name as
a persistent attribute inherited from another entity or mapped
superclass, the behavior is undefined and is not portable between
providers.

- When `FIELD` access is used, as defined above in <<a113>>,
  persistent fields declared by distinct classes are always
  considered distinct attributes.
- When `PROPERTY` access is used, as discussed in
  <<property-access>>, persistent properties declared by
  distinct classes are considered distinct if and only if the
  property accessors of the persistent property of the subtype
  do not override the property accessors of the supertype.

When a persistent property of a subclass overrides a persistent
property of a superclass, the property of the subclass inherits
its mapping from the property of a superclass. The behavior when
mapping annotations are placed in the property of the subclass
is undefined.
...
public class Employee extends Person {
    ...
    // Allowed: mapping is inherited from Person.getName().
    @Override
...
public class Employee extends Person {
    ...
    // Undefined: mapping annotations on overriding property
...
Two persistent attributes (declared by the same entity class or
by distinct entity classes) are permitted to map the same column
of a given table if and only if:

- the persistent attributes have exactly the same type, and
- either the two attributes are declared in distinct branches of
  the entity class hierarchy, so that no entity class features
  both attributes, or one of the persistent attributes is declared
  with `insertable=false, updatable=false`, as defined later in
  <<metadata-annotations>>.

It is the responsibility of the application developer to satisfy
this restriction. In an entity class hierarchy mapped via the
single table strategy or table per concrete class strategy, the
developer might need to explicitly specify distinct column mappings
for persistent attributes with identical names.

The behavior is undefined and unportable if:

- any two distinct persistent attributes with distinct types map
  the same column of a given table, or
- two distinct persistent attributes declared or inherited by
  a given entity class map the same column of a given table and
  neither is mapped `insertable=false, updatable=false`.
...
@Entity
public class Employee extends Person {
    @Column(name="EMPLOYEE_CODE", length=8)
    private String code;
}
...
@Entity
public class Customer extends Person {
    // Disallowed: maps to the same CODE column as Employee.code
    private int code;
}
----
...
public class Job {
    ...
    // Allowed: read-only copy of STATUS
...
This specification defines many annotations with elements referencing
database objects by name, and rules defining the default name when such
an annotation element is not explicitly specified. These names include
the names of tables, columns, sequences, stored procedures, and so on.

By default, the name of a database object must be treated as an
undelimited (unquoted) identifier and passed to the database as such.

For example, assuming the use of an English locale, the following
table names must be passed to the database as undelimited identifiers
so that they will be treated as equivalent by all databases that comply
with the requirements of the SQL Standard for the treatment of
“regular identifiers” and “delimited identifiers” <<a19494>>:
...
Similarly, the following column names should be treated as equivalent:
...
This behavior relieves the developer of the need to explicitly specify
a table or column name when the name differs only in case from the
defaulted name derived from the name of the mapped entity or attribute.

There are two ways to specify that a delimited (quoted) identifier
should be passed to the database.

- The `<delimited-identifiers/>` element within the
  `<persistence-unit-defaults>` element of the object/relational XML
  mapping file specifies that all database identifiers in use by a
  persistence unit must be treated as delimited identifiers. If the
  `<delimited-identifiers/>` element is specified, it cannot be
  overridden.

- Alternatively, when the `<delimited-identifiers>` element is not
  specified, and the name of a database object given by an annotation
  member or in an XML mapping is enclosed within escaped double quotes,
  for example,

  * `@Table(name="\"customer\"")`, or
  * `<table name="\&quot;customer\&quot;"/>`,

+
the name is treated as a delimited identifier.

When `<delimited-identifiers>` is specified, every escaped double quote
within an annotation member or XML element or attribute is interpreted
literally as part of the name of the database identifier.
