...
specified, an instance of type `Object` is returned for each result
of the query execution. If multiple selection items are specified, an
instance of type `Object[]` is instantiated and returned for each
...
The `join` methods have the same semantics as a join in Jakarta Query.
...
be referenced elsewhere by the query, as described in <<fetch-joins>>.
...
Applications that use multi-level fetch joins are not portable.
...
The semantics of path navigation are defined in <<path-expressions>>.
...
string, the value an object. The result of this query is returned
...
The semantics of the conditional methods -- for example, `equal`, `notEqual`,
`gt`, `ge`, `lt`, `le`, `between`, and `like` -- mirror those of the
corresponding operators in Jakarta Query.

Compound predicates are constructed by means
of the `and`, `or`, and `not` methods of the `CriteriaBuilder`
interface.

The types of conditional operands must satisfy the restrictions specified
for the corresponding operators defined by Jakarta Query.
Null value semantics are defined in <<null>> and <<ternary-logic>>.
Equality and comparison semantics are described in <<equality>>.
...
involving these methods. The following rules match to the rules for
numeric type promotion in Jakarta Query.
...
[NOTE]
====
The semantics of the SQL division operation are not standard across databases.
In particular, when both operands are of integral types, the result of the division
operation is an integral type in some databases, and a non-integral type in others.
Portable applications should not assume a particular result type.
====
...
Applications that do not use one of these methods are not portable.
====
...
interface specifies that duplicate values must be eliminated
...
The semantics of the `construct` method used in the selection list are
described in <<a5500>>. The semantics of embeddable types in the
selection list are described in <<a5511>>.
...
The first of these queries returns orders
that are not associated with customers, whereas the second does not. The
...
items are as specified by Jakarta Query.
...
Null values are ordered according to the rules defined by the Jakarta
Query specification.
...
As specified in <<a5636>>, the persistence context is not automatically
synchronized with the result of the bulk update or delete.
