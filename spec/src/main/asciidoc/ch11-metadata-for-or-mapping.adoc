...
The `MapKeyColumn` annotation is used with owned collections. It is specified
on the owning side of a one-to-many or many-to-many relationship or on an
element collection. The `MapKeyColumn` annotation is specified on the side of
the relationship that references the map. The map key is not visible as part
of the state of the entity or embeddable class.
...
an element collection. The `MapKeyJoinColumn` annotation is specified on the
side of the relationship that references the map. The map key is not
...
The `OrderColumn` is used with owned collections. This annotation may be
specified on an owned one-to-many or many-to-many relationship or on
an element collection. The `OrderColumn` annotation is specified on the
side of the relationship that references the collection that is ordered.
The order column is not visible as part of the state of the entity or
embeddable class.footnote:[The `OrderBy` annotation should be used for
ordering that is visible as persistent state and maintained by the
application.]
