...
The `FetchType` enum defines policies for fetching associated data from
the database:
...
- The `EAGER` policy is a requirement on the persistence provider
  runtime that data must be eagerly fetched.

- The `LAZY` policy is a _hint_ to the persistence provider runtime
  that data should be fetched lazily when it is first accessed, in the
  case of a managed entity associated with a persistence context, or
  on a call to the `fetch()` method of `EntityAgent`, in the case of a
  detached entity. The implementation is permitted to eagerly fetch
  data for which the `LAZY` policy hint has been specified.footnote:[In
  particular, lazy fetching might only be available for `Basic` mappings
  for which property-based access is used.]

- A `DEFAULT` value specifies that the fetching policy depends on the
  _default fetch type of the persistence unit for one-to-one and
  many-to-one associations_. This default fetch type is controlled via
  the `default-to-one-fetch-type` element in the `persistence.xml` file
  or by setting the `defaultToOneFetchType` of the
  `PersistenceConfiguration` object used to bootstrap the persistence
  unit, and defaults to `EAGER` for backward compatibility.

[NOTE]
It is very strongly recommended that new applications set the default
fetch type for one-to-one and many-to-one associations to `LAZY` to
avoid accidental fetching of unnecessary data.
...
The `Access` annotation specifies an
...
The `foreignKey` element specifies
or controls the generation of a foreign key constraint for the columns
...
policy is a requirement on the persistence provider runtime that the
value must be eagerly fetched. The `LAZY` policy is a hint to the
...
The `Cacheable` annotation specifies whether an entity should be cached
in the second-level cache, as defined in <<a3061>>, when:

- the persistence provider offers a second-level cache,
- caching is enabled, and
- the `SharedCacheMode` specified by the `shared-cache-mode` element
  in `persistence.xml`, by the `sharedCacheMode()` of the
  `PersistenceConfiguration` object, or by the configuration property
  `jakarta.persistence.sharedCache.mode`, is set to `ENABLE_SELECTIVE`
  or `DISABLE_SELECTIVE`, as described in <<a3065>>.

The value of the `Cacheable` annotation is inherited by subclasses;
it can be overridden by explicitly specifying `Cacheable` on a subclass.
...
A persistence provider is never _required_ to store an entity in the
second-level cache.

- `Cachable(true)` (or simply `Cacheable`) means that the entity and its
  state may be cached in the second-level cache, at the discretion of the
  provider, even if `ENABLE_SELECTIVE` is specified or if the superclass
  of the entity is not cacheable.

- `Cacheable(false)` means that the entity and its state must not be cached
  in the second-level cache, even if `DISABLE_SELECTIVE` is specified, or if
  the superclass of the entity is cacheable.

When the shared cache mode is `ALL` or `NONE`, the `Cacheable` annotation
must be ignored by the provider.

The semantics of the `Cacheable` annotation are undefined when:

- the `shared-cache-mode` element is not specified in the `persistence.xml`
  file or the `sharedCacheMode()` of the `PersistenceConfiguration` is
  `UNSPECIFIED`, and
- the property `jakarta.persistence.sharedCache.mode` is not specified
  when the entity manager factory for the persistence unit is created.

In this case, the behavior is specific to the persistence provider.
...
The `CheckConstraint` annotation is used to specify a SQL check
constraint on a column or table when schema generation is in effect.
...
The `name` element specifies a name for the check constraint.

The `constraint` member specifies a SQL expression written in the
native SQL dialect of the database.

.CheckConstraint Annotation Elements
[#a1224,options="header"]
|===
|Type |Name |Description |Default
...
[NOTE]
The semantics of the second-level cache are extremely vendor-specific.
Portable applications should not rely on any specific behavior connected
with the second-level cache. Most implementations of this specification
require additional configuration beyond the use of the `Cacheable`
annotation to make use of the second-level cache. This specification does
not even require that a persistence provider provide a second-level
cache.
...
The `Column` annotation specifies a
...
are converted to a basic type:

- enabling a converter which was defined `autoApply=false`,
- overriding the use of a converter which was defined `autoApply=true`
  (see <<a13903>>), or
- overriding the use of a converter specified by a field or property of
  an embedded type or inherited mapped superclass.

When property access is used, the `Convert` must be applied to the getter
method.
...
Depending on the inheritance mapping strategy, as defined in <<a966>>,
for an entity class hierarchy, a discriminator column might be used to
determine the entity subclass which represents a given row in the
database.

- For the `SINGLE_TABLE` mapping strategy, a discriminator column is
  always necessary.
- For the `JOINED` strategy, some persistence providers require the
  use of a discriminator column.
- For the `TABLE_PER_CLASS` strategy, a discriminator column is never
  necessary.

The `DiscriminatorColumn` annotation declares a discriminator column.

The inheritance mapping strategy, and discriminator column, if any, are
determined by the mapping metadata for the root entity of an entity class
hierarchyfootnote:[Or of the subhierarchy in which a distinct inheritance
mapping strategy is used. The combination of inheritance strategies within
a single entity inheritance hierarchy is not defined by this specification.]
Therefore, the `DiscriminatorColumn` annotation must be applied to a root
entity class in an entity class hierarchy, even if the root class is an
`abstract` class.

Every discriminator column has a well-defined type. The discriminator
types allowed by this specification are enumerated by `DiscriminatorType`:
...
If the type of a discriminator column is not explicitly specified by the
`DiscriminatorColumn` annotation or equivalent XML element, the type
defaults to `STRING`.

If the `DiscriminatorColumn` annotation is missing, and a discriminator
column is required by the mapping strategy and persistence provider:

- the name of the discriminator column defaults to `DTYPE` and
- the discriminator type defaults to `STRING`.

<<a14548>> lists the annotation elements that may be specified for the
`DiscriminatorColumn` annotation, along with their default values.

The type of the discriminator column, if specified in the optional
`columnDefinition` element, must be consistent with the discriminator
type.
...
The `DiscriminatorValue` annotation specifies the value of a
discriminator column for rows represented by entities of a given type.
This annotation must be applied to a concrete entity class.

If the `DiscriminatorValue` annotation is not specified for a given
concrete entity subclass in an entity inheritance hierarchy which
makes use of a discriminator column:

- if the discriminator type is `STRING`, the discriminator value defaults
  to the entity name, as defined in <<entity-annotation>>, or
- otherwise, a provider-specific function is used to generate a default
  value representing the entity type.

A portable application should explicitly specify the `DiscriminatorValue`
for every entity subclass when the discriminator type is anything other
than `STRING`.

The inheritance mapping strategy, and discriminator column, if any, are
determined by the mapping metadata for the root entity of an entity class
hierarchyfootnote:[Or of the subhierarchy in which a distinct inheritance
mapping strategy is used]. On the other hand, the discriminator value, if
not defaulted, must be specified for every entity class in the hierarchy.

The `DiscriminatorValue` annotation has only a `value` member, as listed
below in <<a14585>>. A discriminator value must be consistent in type with
the discriminator type (see <<a14530>>) of the discriminator column for the
entity class hierarchy. The `value` member of `DiscriminatorValue` accepts
a string which is interpreted as a literal value of the discriminator type.
If the discriminator type is `INTEGER`, the specified `value` string must
be convertable to an integer value by calling `Integer.parseInt()`.
...
The `ElementCollection` annotation declares an element collection, as
defined in <<a494>>. The elements of the collection must be instances
of a basic type or embeddable class.

The `ElementCollection` annotation (or equivalent XML element) must be
explicitly specified if such a collection maps to a collection
table.footnote:[If it is not specified, the rules of <<a511>> apply.]
...
<<a14614>> lists the annotation elements that may be specified for the
`ElementCollection` annotation, along with their default values.
...
lazily loaded or must be eagerly fetched. The `EAGER` policy is a
requirement on the persistence provider runtime that the collection
elements must be eagerly fetched. The `LAZY` policy is a hint to the
persistence provider runtime.
|LAZY
|===
...
The `Embeddable` annotation declares that the annotated class is an
embeddable class, as defined in <<a487>>, whose instances are stored
as an intrinsic part of an owning entity and share the persistent
...
The `Embedded` annotation specifies that a persistent field or property
of an entity or embeddable class is an instance of an embeddable
class.footnote:[If the embeddable class is used as a primary key, the
`EmbeddedId` rather than the `Embedded` annotation must be used.] Each
of the persistent properties or fields of the embedded object is mapped
to the database table for the entity or embeddable class. The embeddable
class must be annotated as `Embeddable`.footnote:[Use of the `Embedded`
annotation is not required. See <<a511>>.]

The `AttributeOverride`, `AttributeOverrides`, `AssociationOverride`,
and `AssociationOverrides` annotations may be used to override mappings
declared or defaulted by the embeddable class.

An implementation is not required to support embedded objects mapped
across more than one table. An entity which splits the attributes of a
single embeddable object across its primary and secondary tables, or
across multiple distinct secondary tables, is not considered portable.
Therefore, all `AttributeOverride` and `AssociationOverride` annotations
for a given embedded attribute should specify columns belonging to the
same `table`.
...
The `EmbeddedId` annotation identifies a persistent field or property of
an entity class or mapped superclass as holding a composite primary key
represented as an embeddable class, as defined in <<a132>>. The embeddable
class must be annotated as `Embeddable`.footnote:[The `Id` annotation is
not applied to any field or property of the embeddable class.]

An embedded id class must not declare a field or property representing a
relationship, in the sense of <<a516>>, with another entity.

In a given entity class:

- there must be at most one field or property annotated `EmbeddedId`, and
- if there is a field or property annotated `EmbeddedId`, there must be
  no field or property annotated `Id`.

This rule is relaxed in the special case where the `EmbeddedId` represents
the composite primary key of a parent entity in an entity with a derived
identity represented as an `IdClass`, as specified in
<<mapsid-derived-identity>>.
...
annotation to specify a derived primary key, as defined in <<a149>>.

If an entity has a derived primary key, the`AttributeOverride` annotation
may only be used to override those attributes of the embedded id that do
not correspond to the relationship with the parent entity.
...
public class Paycheck {
    ...
    @AttributeOverride(name="period", @Column(name="pay_period"))
    @EmbeddedId
    PaycheckId id;
...
is stored with `STATUS` set to 1 and `PAYSCALE` set to _"JUNIOR"_.
...
occurs as the declared type of an `Enumerated` property or field.
...
==== ExcludedFromVersioning Annotation

The `ExcludedFromVersioning` annotation specifies that mutation of the
annotated field or property does not result in incrementation of the
version field or property of the entity.
...
This annotation indicates that the application tolerates lost updates
to the annotated field or property when concurrent optimistic
transactions modify the same entity instance, and that no
`OptimisticLockException` should occur in this scenario.

[NOTE]
Some relational databases disallow equality comparisons between values
of large object type.
The persistence provider is not required to support updating a detached
entity with an attribute excluded from versioning via an `EntityAgent`
if the entity has state mapped to a column of large object type or of
any other type for which the database rejects equality comparisons.
...
provider's default foreign key strategy is used.
...
values of a primary key. The `GeneratedValue` annotation may be applied
...
The `IdClass` annotation specifies a composite primary key class, as
defined in <<composite-primary-keys>>, for the annotated entity class
or mapped superclass. The `IdClass` annotation is used when--and only
when--the primary key encompasses multiple fields or properties of the
entity, as specified by <<a132>>. If a composite primary key is mapped
by an `EmbeddedId`, the `IdClass` annotation must not be applied to the
entity class.

When a composite primary key class is specified via `IdClass`:

- The `Id` annotation must be applied to the primary key fields or
  properties of the entity class, and
- the names of the fields or properties in the primary key class and the
  ...
  their types must match according to the rules specified in <<a132>> and
  <<a149>>.
...
The `IdClass` annotation has only a `value` member, as listed in <<a14847>>.
The specified class must be a legal primary key class, according to the rules
specified in <<composite-primary-keys>>.
...
primary key, as the primary key index is created automatically,
...
The syntax of the `columnList` element depends on the SQL dialect,
but is usually a comma-separated list of column names, with an
optional ordering specification, `ASC` or `DESC`.
...
|String
|type
|(Optional) A SQL fragment representing the index type, usually inserted before the keyword `type` in the generated DDL which creates this index.
|Nothing inserted

|String
|using
|(Optional) A SQL fragment representing the indexing method. When specified, it is added into the generated DDL which creates this index, usually following a `USING` keyword.
|Nothing appended.
...
The `Inheritance` annotation specifies the inheritance mapping strategy,
as defined in <<a966>>, for an entity class hierarchy. This annotation,
if it is specified, must be applied to the entity class that is the root
of the entity class hierarchy.

The inheritance mapping strategies defined by this specification are
enumerated by the `InheritanceType` enum:
...
If the root entity of an entity class hierarchy is not annotated with
the `Inheritance` annotation, and if no inheritance mapping strategy
is specified for an entity class hierarchy by an XML descriptor, the
inheritance mapping strategy for the entity class hierarchy defaults to
the `SINGLE_TABLE` mapping strategy.

This specification does not require support for mixing inheritance
mapping strategies within a single entity class hierarchy. Therefore,
a portable application must not apply the `Inheritance` annotation to
a non-root entity in an entity class hierarchy.

This specification does not require support for the `TABLE_PER_CLASS`
mapping strategy in this release.
...
that use such mappings are not portable.
...
target of the association. Required if the target entity type cannot be
inferred from the declared type of the annotated field or property, for
example, if the declared type is a raw collection type.
|The type argument of the collection type when the declared type of the
annotated field or property is a non-raw collection type.
...
entity must be eagerly fetched. The `LAZY` policy is a hint to the
persistence provider runtime.
|LAZY

|String
|mappedBy
|The field or property of the target entity that owns the relationship
and specifies its mapping to the database.
This element is only specified on the inverse (non-owning) side of the
association.
|
|===
...
defined by the `CascadeType` enum defined in <<lifecycle-operations>>.
...
Required if the target entity type cannot be inferred from the declared
type of the annotated field or property.
|The declared type of the annotated field or property.
...
requirement on the persistence provider runtime that the associated
entity must be eagerly fetched. The `LAZY` policy is a hint to the
persistence provider runtime. The `DEFAULT` policy selects between
`EAGER` and `LAZY` depending on the default fetch type of the
persistence unit for one-to-one and many-to-one associations.
|`DEFAULT`

|boolean
|optional
|(Optional) Whether the association is optional. By default, the
association is optional, and the annotated field or property may
be set to a null value. When `optional=false`, a non-null value
must be assigned to the non-optional field or property before the
persistence provider attempts to write the state of entity instance
to the database.

May be used in schema generation to infer that the mapped foreign
key column is not null. Ignored if the field or property is
annotated `@jakarta.annotation.Nonnull`.
|true
|===
...
The `MapKey` annotation specifies
...
`MapKeyColumn` is used with owned collections. It is specified on the
owning side of a one-to-many or many-to-many relationship or on an
element collection--and always on the side of the relationship that
references the map. The map key is not visible as part of the state of
the entity or embeddable class. Use of the `MapKeyColumn` annotation on
the unowned side of a relationship is not portable between persistence
providers.
...
The `MapKeyJoinColumn` is used with owned collections. It is specified
on the owning side of a one-to-many or many-to-many relationship or on
an element collection--and always on the side of the relationship that
references the map. The map key is not visible as part of the state of
...
such mappings are not portable.

The `foreignKey` element specifies
or controls the generation of a foreign key constraint for the map key
...
The `MapsId` annotation designates a `ManyToOne` or `OneToOne` relationship
attribute that provides the mapping for

- an `Id` or `EmbeddedId` attribute of an entity with a derived identity,
  or
- an attribute of the `EmbeddedId` class of an entity with a derived identity,

as defined in <<mapsid-derived-identity>>.

The `value` element listed in <<a15615>> specifies the attribute within a
composite key class to which the relationship attribute corresponds. If
the primary key of the entity is of the same Java type as the primary key
of the entity referenced by the relationship, the `value` attribute is not
specified.
...
|String
|value
|(Optional) The name of an attribute within the composite
key class to which the relationship attribute corresponds.
|The relationship maps the primary key of the entity.
|===
...
null), the remove operation is applied to the entity being
...
operation is applied to the source entity, the remove operation is
cascaded to the relationship target in accordance with the rules of
<<remove>>, (and hence it is not
...
If `orphanRemoval` is `true` and an entity
that is the target of the relationship is removed from the relationship
(by setting the relationship to null), the remove operation is
...
of basic type, the ordering is by value of the basic objects and
...
insertion, deletion, or reordering affecting the list.

`OrderColumn` is used with owned collections. It is specified on the
owning side of a one-to-many or many-to-many relationship or on an
element collection--and always on the side of the relationship that
references the collection that is ordered. The order column is not
visible as part of the state of the entity or embeddable class.
footnote:[The `OrderBy` annotation should be used for ordering that
is visible as persistent state and maintained by the application.]
Use of the `OrderColumn` annotation on the unowned side of a
relationship is not portable between persistence providers.
...
The `foreignKey` element specifies
or controls the generation of a foreign key constraint for the primary
...
inheritance strategy is used, the `Table` annotation specifies a
...
The `Transient` annotation specifies that the annotated property or field of
an entity class, mapped superclass, or embeddable class is not persistent.
...
The `Id` annotation (which may be used
in conjunction with the `IdClass` annotation) declares
...
definitions created are provider-dependent and database-dependent.
Applications that are sensitive to the exact mapping that is used should
