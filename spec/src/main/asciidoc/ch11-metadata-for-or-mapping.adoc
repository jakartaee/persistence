...
The `MapsId` annotation is used to designate a `ManyToOne` or `OneToOne`
relationship attribute that provides the mapping for:

- an `EmbeddedId` primary key,
- an attribute within an `EmbeddedId` primary key, or
- a simple primary key of the parent entity,

as defined in <<a149>> and <<a155>>.
