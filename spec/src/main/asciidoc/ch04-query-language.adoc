...
syntax is borrowed from ANSI SQL. Providers are not required to
support right joins or full joins on databases which do not natively
support these join types.
...
, _pass:[<=]_, _<>_ (not equal), _[NOT]_ _BETWEEN_, _[NOT]_ _LIKE_, _[NOT]_
...
- `p.age BETWEEN 15 and 19` is equivalent to `p.age >= 15 AND p.age pass:[<=] 19`
...
conditional expressions are =, <, pass:[<=], >, >=, <>. The result of the
