...
An entity is designated in a query by its _entity name_. The entity
name is specified by the `name` element of the `@Entity` annotation,
as defined in <<entity-annotation>>, or by the `entity-name` XML
descriptor element, and defaults to the unqualified name of the entity
class.footnote:[The unqualified name of a nested entity class
`org.example.Outer.Inner` is just `Inner`.]

Entity names are scoped within the persistence unit and must be unique
...
that contain subqueries on both sides of a comparison operation are
...
Although reserved literals appear in uppercase, they are case-insensitive.
...
`function_invocation` syntax are not portable across databases.
...
The `ELSE` branch of a case expression is optional. A case expression
without an `ELSE` branch is interpreted as if it had the `ELSE` branch
`ELSE NULL`.
...
The `DISTINCT`
keyword specifies that duplicate values must be eliminated from
the query result.
...
embeddable instance returned by the query is not in the managed
state (i.e., it is not part of the state of any managed entity).
...
of a query using an aggregate function is determined as follows:
